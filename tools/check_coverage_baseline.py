...
`go test -count=1 -coverprofile` and compute the module's exact total
statement coverage), store (tools/coverage-baselines.json, committed,
regenerated by --update), compare (fail when the measured total
breaches either bound), and self-check the baseline file's own health
(--selfcheck).
...
module
  ...
  * The rule is per module and both floor- and baseline-relative, so
    ...
    suite (`go test -count=1 -coverprofile ... ./...` from the module
...
def measure_module_coverage(root: pathlib.Path, module_dir: str) -> float:
    ...
    try:
        proc = subprocess.run(
            ["go", "test", "-count=1", "-coverprofile=%s" % profile, "./..."],
            cwd=module_path,
            capture_output=True,
            text=True,
            check=False,
