...
module-level regression guard `go/ratelimit/unittest/no_cjk_characters_test.go`,
which uses `go/parser` + `unicode.Is(unicode.Han, r)`; this script reproduces
...
| `.idea/`, `.vscode/` | IDE-local directories holding developer machine state (UI strings etc.); gitignored, never present in CI. |
| `.claude/worktrees/` | This repository's git worktrees — each a complete checkout carrying its own `docs/internal/` Chinese content under a path the `docs/internal/` carve-out above cannot reach. Pruned by exact repo-relative path, never by basename: a directory merely named `worktrees/` is scanned like any other, and `.claude/skills/**` stays scanned source. Gitignored local machine state, never present in CI. |
...
Scaffolds the canonical Go-module stub — exactly the three files a stub
module under `go/` carries: `go.mod` (`module github.com/vislake/speed/go/<name>`
...
scaffolding, a registration checklist prints (go.work `use` entry — the
module's registration for the release coordinator's tag list and for the
derived CI module sets alike, both of which follow go.work at runtime —
the coverage-baseline recording follow-up, the integration-tier rows when
the module ships one, roadmap and navigation rows) as reminders — the
script never edits those shared files itself, because a scaffolder that
silently rewrites `go.work` and CI matrices makes review diffs
unreadable; the checklist is the contract for the human or for the
...
`--category npm` scaffolds the `@speed/<name>` package skeleton and
`--category app` an application-side module, each with its own checklist
(the table row above describes both); `--dry-run` prints the plan
...
| `handwritten-tenant-id-filter.yml` | no hand-written `WHERE tenant_id = ?` | a `Where` / `Or` / `Not` / `Having` chain call whose FIRST argument is a string literal containing a `tenant_id = ?`-style clause | test files; `go/dbkit/**` (the scoping plugin builds the filter everyone else relies on); `go/jobs/store.go` (platform-data idempotency guard); `go/config/store.go` (the configs-table accessor's exact `(scope, tenant_id, key)` lookups on platform data -- the model implements no `TenantScoped`, so no injected filter exists for this table, and ScopeSystem rows carry an empty tenant_id); `go/billing/plan.go` and `go/ai-gateway/store.go` (the same dual-domain shape on `billing_plans`/`ai_gateway_credentials`: a platform-wide row must stay visible to every tenant's lookup, so the models implement no `TenantScoped` either and the `(tenant_id, ...)` lookups are the lookup-precedence mechanism); `go/admin/tenant_repository.go` (the admin tenant ledger, whose primary-key column is named `tenant_id` -- ordinary primary-key lookups); `go/metering/repository.go` and `go/notification/send_record.go` (platform-data `(tenant_id, idempotency_key)` idempotency lookups); the `internal/testutil` / `internal/apptest` test-support packages wholesale (the same testing surface as test files in a different file shape: shared test helpers other packages' tests import, so they cannot carry the `_test.go` suffix; the premise they rest on is enforced in reverse by the host-composition gate's production-import check) | a clause assembled dynamically (fmt.Sprintf into the clause, a filter passed through a helper); only the first-argument literal form is matched |
