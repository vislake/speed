...
| `raw-gorm-bypass.yml` | no `db.Table` / `db.Model` / `db.Raw` around the Repository | any call to the three bypass entry points on any receiver, in shipped code | test files; `go/dbkit/**` (dbkit owns the raw surface it provides); `go/jobs/store.go` (platform data whose dispatch query must scan every tenant); `go/authn/internal/testutil/testutil.go` (a test fixture that cannot be named `*_test.go` itself, since authn's own external test files import it) | a workaround through another `*gorm.DB` method (e.g. `Exec` with hand-written SQL) is not matched |
