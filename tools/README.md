...
| `check_coverage_baseline.py` | Checker | docs/internal/20-quality-and-security.md's "coverage must not decline (against a baseline)" rule for the six foundation modules (pkgcore, dbkit, tenancy, rbac, billing, jobs): measures a module's exact unit-suite statement coverage from a `go test -coverprofile` run and compares it with its committed `tools/coverage-baselines.json` row inside a documented tolerance; `--update` records new baselines (a deliberate decline must ride the same change that records it, with the reason in the commit); `--selfcheck` keeps the baseline file itself complete and current (every row a live foundation module, every foundation module with a row) so a stale row cannot silently exempt a module | 0 clean / 1 decline beyond tolerance, stale/missing row, or toolchain moved / 2 usage error |
