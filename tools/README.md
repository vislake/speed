...
| `check_host_core_parity.py` | Checker | Shared host kernel: package `hostcore` (the liveness endpoints, the pre-auth allowlist set, the route-mount rule, the mounted-route label seed and the serve/shutdown lifecycle) exists as one file carried byte-identically by the reference app (`examples/reference-app/internal/hostcore/hostcore.go`) and the saasctl template (`go/saasctl/internal/template/project/internal/hostcore/hostcore.go`, the copy `saasctl new` materializes into every generated project), with neither host allowed to re-declare its symbols or re-grow its statements (the serve loop, the `/healthz`, `/metrics`, `/api/v1/authn` literals) outside that copy. Planted-drift suite: `tools/test_check_host_core_parity.py` (run in the same step pair in fast-check's repo-checks job) | 0 clean / 1 finding / 2 usage error |
| `check_env_example_consistency.py` | Checker | Reference-app env-example consistency: the env variables the host's loader target declares (`examples/reference-app/internal/app/bootstrap.go`'s `config:"env=NAME"` pins -- the `,derive` option included -- plus the `APP_ROOT_KEY` literal the loader reads through `WithRootKeyEnv`) must equal the key set `examples/reference-app/.env.example` documents, its active `KEY=` entries and its commented-out examples alike (a prose mention is not an entry); both directions fail, and the example-side one is strict with no in-file escape marker -- the file's declared scope is exactly the host's surface, so a non-host entry is a stale line to remove or a variable to pin in the loader target. Planted-drift suite: `tools/test_check_env_example_consistency.py` (run in the same step pair in fast-check's repo-checks job) | 0 clean / 1 finding / 2 usage error |
