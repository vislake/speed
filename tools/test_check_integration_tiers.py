...
test), and a Taskfile.yml fragment with the INTEGRATION_DIRS variable.
Each test plants one disagreement between the tree and the copy and
pins the gate's verdict: the clean fixture stays green; a missing
entry, an extra entry, a duplicated name, a stale exclusion and an
exclusion that reappeared in the copy each fail as drift (exit 1 path);
a Taskfile in a shape the reader does not recognize and a non-module
entry each fail as infrastructure (exit 2 path). The final case runs
the gate against this repository's real tree, so an interface drift
between the gate and the real lockstep-release.py / Taskfile.yml
surfaces here.
"""
...
class FixtureCase(unittest.TestCase):
    ...
    examples/app consumer entry and the copy wired to go/alpha."""
    ...
    def _check(self) -> tuple[list[str], list[str], list[str]]:
        return m.check(str(self.root))

    def _problems(self) -> list[str]:
        problems, _, _ = self._check()
        return problems
...
class ExtraInCopy(FixtureCase):
    ...
    def test_negated_constraint_does_not_count_as_a_tier(self) -> None:
        # go/beta is tagged OFF under the integration tag; the copy
        # naming it is drift, and the derived set is unchanged.
        self._write_fixture(taskfile_dirs=["go/alpha", "go/beta"], beta_negated=True)
        problems, tiers, _ = self._check()
        self.assertEqual(tiers, ["go/alpha", "go/saasctl"])
        self.assertEqual(len(problems), 1)
        self.assertIn("go/beta", problems[0])

    def test_post_package_mention_does_not_count_as_a_tier(self) -> None:
        # go/gamma was never given a copy entry, so a green result proves
        # the after-package mention did not enter the derived set; the
        # entry-naming-gamma case proves the other direction.
        self._write_fixture(gamma_after_package=True)
        problems, tiers, _ = self._check()
        self.assertEqual(problems, [])
        self.assertNotIn("go/gamma", tiers)

    def test_entry_naming_a_post_package_mention_module_is_drift(self) -> None:
        self._write_fixture(
            taskfile_dirs=["go/alpha", "go/gamma"], gamma_after_package=True
        )
        problems, tiers, _ = self._check()
        self.assertNotIn("go/gamma", tiers)
        self.assertEqual(len(problems), 1)
    ...
    def test_duplicate_copy_entries_are_drift(self) -> None:
        self._write_fixture(taskfile_dirs=["go/alpha", "go/alpha"])
        problems = self._problems()
        self.assertEqual(len(problems), 1)
...
class DeclaredExclusion(FixtureCase):
    def test_exclusion_in_the_copy_is_drift(self) -> None:
        self._write_fixture(taskfile_dirs=["go/alpha", "go/saasctl"])
        problems = self._problems()
        self.assertEqual(len(problems), 1)
...
class ReaderShapeErrors(FixtureCase):
    ...
    def test_non_module_entry_is_infrastructure_error(self) -> None:
        self._write_fixture(taskfile_dirs=["go/alpha", "web/packages/x"])
        message = self._infra_error()
...
class RealTree(unittest.TestCase):
    def test_real_repository_is_green(self) -> None:
        # Exercises the real lockstep-release.py and Taskfile.yml through
        # this gate, so an interface drift between them surfaces here.
        root = pathlib.Path(__file__).resolve().parent.parent
        problems, tiers, dirs = m.check(str(root))
        self.assertEqual(problems, [], "\n".join(problems))
        self.assertIn("go/sharing", tiers)
        self.assertIn("go/dbkit", tiers)
        self.assertNotIn("go/saasctl", dirs)
        # Membership, not order: the Taskfile loop is read in its own
        # order; the derived set is compared as a set (check() already
        # pins membership exactly).
        self.assertEqual(
            sorted(dirs), sorted(t for t in tiers if t not in m.EXCLUDED)
        )
