...
An end-to-end suite with recorded/replayed AI responses — real provider calls happen only during milestone acceptance and demos — is planned to run in CI with the e2e pipeline (`.github/workflows/e2e.yml`, a gated stub deferring to the roadmap M4 e2e item); no live workflow runs a reference-app suite today (its unit suite runs on `full-ci`-labeled PRs via pr-full).
