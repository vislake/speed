...
A **modular monolith distributed as libraries**. This is the single most important thing to internalize: speed is not an application, it is 21 independently released Go modules plus 16 npm packages that business projects pull in via `go get` / `npm install`. They compile into one binary and call each other in-process — no service discovery, no Kubernetes-shaped infrastructure. Only a minimal starter skeleton is generated by CLI and freely editable by consumers.
...
**Lockstep**: all 21 Go modules and 16 npm packages share one version number and release together; only same-version combinations are supported. This removes the compatibility matrix entirely, at the cost of consumers upgrading everything at once (`saasctl upgrade` handles the rewrite).
