...
func TestNotesDeleteRestore_HTTPLifecycle(t *testing.T) {
	...
	resp := noteMutationRequestAs(t, srv, http.MethodPost, acmeToken, "/"+noteID+"/restore", demoOwnerUserID)
	assertNoteNotFound(t, resp, "restore of a live note")

	// Delete: 204, and the note leaves the tenant's list.
	resp = noteMutationRequestAs(t, srv, http.MethodDelete, acmeToken, "/"+noteID, demoOwnerUserID)
	resp.Body.Close()
	...
	resp = noteMutationRequestAs(t, srv, http.MethodDelete, globexToken, "/"+noteID, demoOwnerUserID)
	assertNoteNotFound(t, resp, "another tenant's delete of acme's note")

	// Unknown id: the same uniform 404.
	resp = noteMutationRequestAs(t, srv, http.MethodDelete, acmeToken, "/00000000-0000-0000-0000-000000000000", demoOwnerUserID)
	...
	resp = noteMutationRequestAs(t, srv, http.MethodPost, globexToken, "/"+noteID+"/restore", demoOwnerUserID)
...
func TestNotesDelete_RbacGate_AnswersForTheNewVerb(t *testing.T) {
	...
	resp := noteMutationRequestAs(t, srv, http.MethodDelete, readerToken, "/"+noteID, demoReaderUserID)
	...
	resp = noteMutationRequestAs(t, srv, http.MethodPost, readerToken, "/"+noteID+"/restore", demoReaderUserID)
