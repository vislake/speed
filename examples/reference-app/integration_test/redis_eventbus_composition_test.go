...
// is the proof that the APP_REDIS_ADDR composition really works
...
// (internal/app/demo/demo_users.go) registers demoOwnerEmail as a real account
// with real
...
// checked, none of it faked. The signing key is generated by go/pki's
// LocalSigner with no seed this process could derive independently --
// demoAccessToken's own doc comment has the full story -- so the real
// login round trip through the child's own HTTP stack is the only
// workable way to obtain a token, exercising the account-seeding path
// and the sign-in path the whole way. The tenant the
...
// (internal/app/demo/demo_subject.go's DemoSubjectResolver).
//
...
// DemoSubjectResolver (internal/app/demo/demo_subject.go) reads the acting user
...
const (
	...
	// demoOwnerEmail is internal/app/demo/demo_users.go's DemoOwnerEmail, copied
...
//
// A hand-derived token is impossible here: the signing key authn uses is
// generated by go/pki's LocalSigner (crypto/rand, no seed control) and
// persists inside the CHILD's own SQLite file -- there is no key left for
// this process to derive independently. Driving a real login through the
...
func TestServer_RealRedisEventBusComposition_NotesAuditEventCrossesProcesses(t *testing.T) {
	...
	// own boot-time Grant
	// (internal/app/demo/demo_credits.go) is itself a real CreditService.Grant
