...
**A database written under the old derivation purposes does not carry over.** The derivation purpose now embeds each declared key path (`speed.config.master_key.v1` and its five siblings), while an earlier, unreleased build derived the same six materials under its own `speed.reference-app.*.v1` purposes — so values such an earlier build sealed under `APP_ROOT_KEY` (authn's encrypted PII and blind indexes, pki's persisted signing key, config's sealed values) do not decrypt under the material this build derives. No published version carries the old purposes, so no supported deployment exists to migrate; a local development database an older checkout created with `APP_ROOT_KEY` set should be recreated rather than upgraded, while a zero-configuration database still carries over unchanged (the committed development defaults did not move).
