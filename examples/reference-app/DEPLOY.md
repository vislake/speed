...
**A second, unrelated demo affordance this app ships that `APP_DEMO_USERS_PASSWORD` does NOT control**: the `X-Demo-User` request header (`demo_subject.go`'s `demoUserHeader`), read by every permission-gated route regardless of that variable's setting. Any caller who can reach this deployment can set this header to `demo-owner` (or any other seeded demo actor) and act with that actor's full rbac grants — and, worse, it still outranks a REAL, verified access token's own identity when both are sent on the same request, so a real signed-in user's session does not even protect against it. Set `fly secrets set APP_DISABLE_DEMO_USER_HEADER=1` for any deployment a real, non-demo user might reach — this makes every gated route resolve its acting user from the verified access token alone, the header no longer read at all. This deployment's `fly.toml` leaves it **unset entirely** by default, matching every other demo affordance's own opt-in-to-disable posture in this section; a genuinely public, non-demo deployment should set it.
