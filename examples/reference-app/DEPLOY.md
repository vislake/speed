...
**A database written under the old derivation purposes does not carry over.** The derivation purpose embeds each declared key path (`speed.config.cipher_key.v1` and its five siblings), and the purpose set changed more than once during development — earlier, unreleased builds derived the same six materials under earlier purpose sets (the reference app's own `speed.reference-app.*.v1` names among them) — so values such an earlier build sealed under `APP_ROOT_KEY` (authn's encrypted PII and blind indexes, pki's persisted signing key, config's sealed values) do not decrypt under the material this build derives. No published version carries any of the old purposes, so no supported deployment exists to migrate; a local development database an older checkout created with `APP_ROOT_KEY` set should be recreated rather than upgraded, while a zero-configuration database still carries over unchanged (the committed development defaults did not move).

**Why this matters before real public traffic, not just as a convenience.** Before `APP_ROOT_KEY` existed, three of these six key materials — `APP_PKI__LOCAL_KEY_CIPHER_KEY`, `APP_AUTHN__BLIND_INDEX_KEY` and `APP_AUTHN__PII_CIPHER_KEY` — had **no environment-variable override path at all**: the only way to change them was editing the hardcoded, committed-to-source-control development defaults in `internal/app/devkeys.go` itself. A reference-app deployment that set only the three older keys (`APP_CONFIG__CIPHER_KEY`/`APP_ORG__INVITATION_EMAIL_INDEX_KEY`/`APP_NOTIFICATION__CONTACT_INDEX_KEY`) was, without anyone necessarily realizing it, still running its `pki` signing-key storage, its `authn` email/phone blind index and its `authn` PII encryption (email, phone, TOTP secrets) on keys committed to this very repository's source — a real, live gap that round closed. Setting `APP_ROOT_KEY` (or, at minimum, the three individual variables in the table below) before real public traffic reaches a deployment is what actually closes it.
...
Advanced: setting one or more of the six individual keys directly, instead of (or in addition to) `APP_ROOT_KEY`, for fine-grained per-key rotation. **An explicitly-set individual variable always wins over what `APP_ROOT_KEY` would derive for that same key** — the loader applies that precedence per field, and the two compose freely, so a deployment can derive five keys from the root and rotate the sixth independently by setting only its own variable (which must be exactly 64 hex characters, like every explicit key-material value). Each variable's name is the loader's own derivation from the key path its module declares, the path's dot spelled as a double underscore under the `APP_` prefix — so `config.cipher_key` reads `APP_CONFIG__CIPHER_KEY` and `authn.blind_index_key` reads `APP_AUTHN__BLIND_INDEX_KEY`.

| Secret | Why it is sensitive |
|---|---|
| `APP_CONFIG__CIPHER_KEY` | The AES cipher key `config.WithCipher` seals every Sensitive dynamic-configuration value with. The key that encrypts the `configs` table cannot live in that table, so it must come from the environment — and a real deployment must never fall back to the committed, documented-as-non-secret `DevConfigKey` development default. |
| `APP_ORG__INVITATION_EMAIL_INDEX_KEY` | The HMAC key org's blind indexer normalizes and indexes invitation email addresses with. A dbkit rule (an AES key must never double as an HMAC key) is why this is a separate secret from `APP_CONFIG__CIPHER_KEY`, never the same value. |
| `APP_NOTIFICATION__CONTACT_INDEX_KEY` | The HMAC key the notification module's blind indexers index encrypted contact email/phone addresses with. Same separate-secret rule as above, and separate again from `APP_ORG__INVITATION_EMAIL_INDEX_KEY`. |
| `APP_PKI__LOCAL_KEY_CIPHER_KEY` | The AES key that seals go/pki's `LocalSigner` private-key column — the key authn's access tokens are ultimately signed with. Had no override path before `APP_ROOT_KEY` existed; a deployment that never set this was running signing-key storage on a key committed to this repository. |
