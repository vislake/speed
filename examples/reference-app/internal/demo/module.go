// Package demo hosts the reference app's demo notification type: the
...
// contact, wired end to end as the notification module's first consumer (see
// cmd/server/demo_notification.go and the module's own notification_flow_test
// suite).
//
// The package exists because the reminder's template copy must live inside
...
// module that qualifies: it declares no tables, no API fragment, no events,
// no permissions and no configuration -- only one notification type and the
// two-language template set that renders it.
//
...
func (m *Module) Register(reg *pkgcore.Registry) error {
	return reg.Notifications.Add(patientReminderNotificationType)
}
