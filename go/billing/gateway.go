...
// go/billing/gateway (and its stripe/alipay/wechat sub-subpackages)
...
// notify param) into billing's own domain model, coupling
...
// depguard rules in .golangci.yml (stripe-sdk-only-in-
...
// than what a subpackage needs, are the other half.
...
const (
	...
	// the explicit non-scope on their native recurring billing) was
	// canceled on the channel's side. ChannelStatus is
	...
	// genuinely delayed settlement) being the one case the shipped
	...
	// channel-side object at this moment -- the agreement point the
...
// billing's own code ever has to understand (a caller driving
// Subscription/Invoice transitions from the event included), normalized
// upward from each channel's own vocabulary. It never carries a
// provider-specific field: no stripe.Event, no
...
type NormalizedEvent struct {
	...
	// natural key the insert-first-dedup rule dedups payment_events on
	// (see PaymentEventRepository.InsertIfNew's own doc comment).
	EventID string
	...
	// NormalizedEventSubscriptionCanceled carries no amount). Because
	// channel callbacks cannot be trusted, this value is only a signal
	// for a caller to decide whether to re-query QueryStatus, never
	// trusted on its own to update a ledger or an invoice -- see
	// QueryStatus's own doc comment.
	Amount Money
...
type ChargeRequest struct {
	...
	// identical reason. Every provider derives its own
...
// comments), so no table in the module's own scope stores it; a live
// webhook endpoint is where that mapping decision gets made (none is
// mounted yet).
...
// declared here in billing's own root package so business code -- a live
// webhook endpoint included -- depends on this interface alone, never on
// go/billing/gateway or any provider SDK. See this file's own header
// comment for the full dependency-direction rationale.
type PaymentGateway interface {
	// CreateCharge asks the channel to create one collection. What that
	// means differs by channel, and is a deliberate, documented pragmatic
	// trade-off, not an abstraction leak:
	//
	...
	//     at this repository's target tier (the surveyed state of both
	//     channels' published APIs), so a caller re-runs CreateCharge once
	//     per internally-tracked billing cycle instead -- Alipay/WeChat
	//     native recurring billing is deliberately not implemented.
	//
	...
	// subpackage does exactly that -- see stripe's own
	...
	// current status and amount of one ChannelReference -- the rule that a
