...
//
// # HTTP surface (read-only, this round)
//
...
// consequence). Register builds Handler -- serving the fragment's two GET
// operations, balance and recent transactions, over CreditService -- and
// mounts it on the host's router at apiPath, the same reg.Routes.Mount
...
// per docs/internal/21-api-contract.md's spec-first decision. The surface
// is read-only this round (balance + recent transactions); see the
// fragment's own header for why no write operation ships yet.
...
func (m *Module) Register(reg *pkgcore.Registry) error {
	...
	// NewModule.
	m.handler = NewHandler(m.credits)
