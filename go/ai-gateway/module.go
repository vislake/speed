...
func (m *Module) Register(reg *pkgcore.ComponentRegistry) error {
	...
	m.gateway.host = reg
	// Attaches the registry as the Gateway's provider resolution source: a
	// route's provider name resolves as a selected member of the
	// "chat"/"image" directories through it from here on (gateway.go's
	// buildChat/buildImage).
