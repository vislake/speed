...
- Do not call `pkgcore.WithSystemContext` directly from code that is able to import `tenancy`. Use `tenancy.WithSystemContext` so the grant is audited. The raw primitive remains correct only for code at or below `tenancy` in the graph, which cannot import `tenancy` at all — plus two documented exceptions whose preconditions are stated in place at each call site: boot-time writes (a process-start declaration re-affirmed on every restart under a fixed actor has no operator session or ticket to attribute an audit row to, so no audit reader has a question its record would answer) and test-support code (an audit event per conformance-suite call is noise, not accountability). Root CLAUDE.md's Multi-tenant isolation section splits the escape hatch's uses the same way: cross-tenant **widening** stays restricted to `admin`/`compliance`/`jobs`/`authn`, while platform-scope write **gates** that never widen anything — `go/config`'s `ScopeSystem` tier, `ai-gateway`'s platform-credential write — belong to the module owning the write path and still use this wrapper wherever it is importable (`ai-gateway`'s request-time platform-credential handler is the proof). `dbkit.HardDelete` is the mechanism-level instance of the same gate, not an allowlist question: dbkit, below `tenancy`, checks presence only, and its grant holders remain the four-module widening whitelist entering through this wrapper.
