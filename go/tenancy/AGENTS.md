...
| Concern | Where |
|---|---|
| `Resolver` interface + `DomainResolver` (host/subdomain-based tenant resolution for unauthenticated requests) | `resolver.go`, `resolver_domain.go` |
...
| `var ErrTenantSuspended = apperr.Forbidden("tenancy.tenant_suspended")` | The structured error `Middleware` writes as a 403 when a `WithTenantStatusResolver`-wired resolver reports the resolved tenant in any state other than `TenantStatusActive` — `TenantStatusSuspended` and any out-of-vocabulary status (the empty status, a case variant, a future third state) reported with a nil error are refused identically, an out-of-vocabulary answer never reading as "assume active". Never returned — never even consulted — on a host that has not wired one |
| `var ErrTenantStatusUnavailable = apperr.Internal("tenancy.tenant_status_unavailable")` | The structured error `Middleware` writes when a wired `TenantStatusResolver`'s `Status` call itself fails. Fail-closed exactly like a resolution failure: an unreachable status source must never be treated as "no news is good news" |
| `type MiddlewareOption func(*middlewareConfig)` | Option type for `Middleware` |
| `func WithAllowlist(method string, paths ...string) MiddlewareOption` | Exempts the given (method, paths) pairs from the refusals `Middleware` would otherwise answer with — on a resolution failure and on a wired tenant-status gate's refusal of the resolved tenant alike — never from resolution or tenant injection itself, so an allowlisted request whose tenant resolves, and passes a wired gate, is injected exactly like a non-allowlisted one. Matching is an **exact string comparison** against both `Method` and `URL.Path` — no prefix, wildcard, case-folding, trailing-slash normalization, or GET-implies-HEAD convenience. The exemption is scoped to `method` as well as path: allowlisting `(POST, "/api/v1/orgs/invite")` does not exempt `GET` on that same literal path — each (method, path) pair needing an exemption gets its own. The exemption covers the tenant-status gate the same way: an allowlisted request whose resolved tenant the status gate refuses — suspended, or any status other than `TenantStatusActive` — or whose `Status` call fails proceeds with **no tenant in its context**, never with the tenant the gate just refused |
| `func AllowlistGETAndHEAD(paths ...string) MiddlewareOption` | `WithAllowlist(http.MethodGet, paths...)` plus `WithAllowlist(http.MethodHead, paths...)` in one call: each named path is exempt in both methods, and in exactly those two — every other method on the same path, and both methods on unnamed paths, keep failing closed. Built for the liveness route set (`/healthz`, `/metrics`, the pre-auth configuration reads), where `ServeMux`'s GET-implies-HEAD convenience serves a HEAD probe from the registered GET pattern while `Middleware`'s exact (method, path) matching would otherwise refuse it. An empty path list exempts nothing, and a genuinely single-method route keeps using `WithAllowlist` directly |
| `func Middleware(resolver Resolver, opts ...MiddlewareOption) func(http.Handler) http.Handler` | Resolves the request's tenant via `resolver.Resolve` and injects it into the request context with `pkgcore.WithTenant`. On failure — including a `Resolver` that reports success with an empty `TenantID`, which is treated identically to a resolution error — responds `ErrTenantUnresolved` (403) unless the (method, path) is allowlisted, in which case the next handler still runs, just with **no tenant in its context**. When `WithTenantStatusResolver` was given, a successfully resolved tenant is additionally checked against it: `TenantStatusActive` is the only status that proceeds (`ErrTenantSuspended` otherwise), and a failing `Status` call refuses closed with `ErrTenantStatusUnavailable` — both honouring the allowlist exactly like a resolution failure |

**Hard rule, not a description: `Middleware` never trusts a header, query parameter, or request body for tenant identity — under any option, in any configuration.** The resolved tenant is the only tenant source downstream code may trust. This is not a convenience default that a caller can opt out of; there is no option on `Middleware` or `MiddlewareOption` that reads a client-supplied tenant hint, and adding one would reintroduce the single most common way multi-tenant systems suffer a horizontal-privilege-escalation breach. A custom `Resolver` (including `authn`'s own authenticated one) must uphold the same rule: derive the tenant from a source the server itself controls — a verified token's claims, a database lookup — never from anything the client attached to the very request being resolved.

### `tenancy` — `tenant_status.go`

| Signature | Purpose |
|---|---|
| `type TenantStatus string` | The closed vocabulary a `TenantStatusResolver` reports — the answer to "is this tenant servable". Only `TenantStatusActive` is servable: `Middleware` refuses any other reported status, including a value this version does not define, so a future status that should let requests through must be added to `Middleware`'s check explicitly; it can never become servable by default |
| `TenantStatusActive` / `TenantStatusSuspended` (`"active"` / `"suspended"`) | The two-value vocabulary: the ordinary state every resolver reports for a tenant nobody has suspended, and the state `Middleware` refuses every request against once a resolver is wired |
| `type TenantStatusResolver interface { Status(ctx context.Context, tenant pkgcore.TenantID) (TenantStatus, error) }` | The OPTIONAL, structurally-typed module interface a host may give `Middleware` (via `WithTenantStatusResolver`) to make a tenant's suspended status actually refuse requests rather than merely being a fact recorded somewhere no request pipeline consults. This package declares the interface and the one call site that consults it; it implements nothing itself — `go/admin`'s tenant ledger (`*TenantService`) is this module's first real implementer, kept entirely on the host side of the boundary, the same "structurally typed, no import in either direction" shape `org.FeatureGate` and `rbac.SubtreeResolver` already use. Status is consulted only for a request whose tenant `Middleware` already resolved successfully — never for an allowlisted request that proceeded with no tenant at all, since there is nothing to look a status up for |
| `func WithTenantStatusResolver(r TenantStatusResolver) MiddlewareOption` | Wires `r` into `Middleware` so a resolved tenant's status is consulted on every request, refusing one against a suspended tenant with `ErrTenantSuspended`. Entirely additive and OFF by default: a host that never calls it gets exactly the pre-module `Middleware` behavior, byte for byte, because nothing consults a status source it was never given — which is what lets the module ship with no back-compat concern |

**The gate fails closed, or it does not run at all.** Wired, `Middleware` consults the resolver synchronously, immediately after resolving a request's tenant and before injecting it into context, and `TenantStatusActive` is the only status that lets the request through. Any other answer reported with a nil error is refused with `ErrTenantSuspended`: `TenantStatusSuspended`, the empty status, a case variant, or a future state this version does not define are all refused identically — an out-of-vocabulary answer never reads as "assume active", the same default-refuse discipline `Middleware` applies to a `Resolver`'s out-of-contract success. A `Status` call that itself errors refuses with `ErrTenantStatusUnavailable` (500): an unreachable status source is an outage to fix, never "no news is good news". Both refusal classes honour `WithAllowlist` the way a resolution failure does — an allowlisted request whose resolved tenant the gate refused proceeds with **no tenant in its context**, never with the tenant the gate just refused. The consequence a host should be able to hold on to: suspension is **not enforced on allowlisted routes** — the allowlist waives the gate's refusal exactly as it waives a resolution failure's, so the enforcement this module buys covers exactly the routes whose (method, path) the host did not allowlist, while a route that works without a tenant in its context — an authentication entry that must work before a tenant is known, say — stays up regardless, letting a suspended tenant's users keep signing in and refreshing tokens through it. The resolver is consulted on every successfully resolved request and nothing is cached, so a suspension takes effect on the very next non-allowlisted request by construction. Caching is deliberately not pre-empted by this module: a cached answer would first have to define how a stale "active" answer is invalidated when a tenant is suspended (see `TenantStatusResolver`'s own doc comment), an invalidation-contract decision for the layer that wants the cache, not a TTL trade-off this package can make on its own. Unwired (the default), the check never runs and `Middleware` is the plain resolution-plus-injection shape in every respect.
...
**What this does NOT do, and must not be assumed to do: it does not widen what `dbkit.Repository[T]` can see or touch.** `Repository[T]` consults `pkgcore.SystemReasonFromContext` in exactly one place, as a refusal gate rather than an amplifier: `HardDelete` (dbkit's system-context-gated physical delete, `hard_delete.go`) requires a system context or refuses with `dbkit.ErrHardDeleteRequiresSystemContext` — and even with the gate passed it stays strictly inside the ctx tenant's rows. Every other `Repository[T]` method never consults `pkgcore.SystemReasonFromContext`, so granting a system context on top of an already tenant-scoped context does not widen what they return beyond that same tenant, and granting one on a context with no tenant at all does not substitute for a tenant either — they still fail closed with `pkgcore.ErrNoTenant`. This is not a safety hole (isolation is never weakened; if anything it is overly conservative), but it is a real functionality gap: no admin cross-tenant read or cross-tenant background job can be built on `Repository[T]` + `WithSystemContext`. The composition options that would close it are tracked as design debt in `docs/internal/17-risks.md`, which also lists what closing either one must additionally satisfy.
...
- Do not have a custom `Resolver` derive a tenant from anything the client controls on the request being resolved. Every implementation, `authn`'s own authenticated one included, must use a source the server itself controls.
...
- Do not expect prefix, wildcard, case-folding, trailing-slash, or GET-implies-HEAD matching from `WithAllowlist`. It is an exact string comparison against both `Method` and `URL.Path` — allowlist `http.MethodHead` explicitly if a health check needs it too, or call `AllowlistGETAndHEAD(paths...)` for the whole GET+HEAD pair at once (the shape a probe- or scraper-fetched liveness route needs, since `ServeMux` serves HEAD from the registered GET pattern while `Middleware` does not).
...
- Do not wire a `TenantStatusResolver` and treat an answer you do not recognize as "assume active". `Middleware` refuses every status other than `TenantStatusActive` with `ErrTenantSuspended` — the empty status, a case variant, a future third state reported with a nil error all refuse identically — and refuses a failing `Status` call with `ErrTenantStatusUnavailable`. A status source that errors is an outage to fix, not a reason to let requests through.
- Do not cache a tenant's status between requests on top of this module. `Middleware` consults the resolver synchronously on every successfully resolved request and caches nothing, so a suspension takes effect on the very next non-allowlisted request; a caching layer must first answer how a stale "active" answer is invalidated when a tenant is suspended (see `TenantStatusResolver`'s doc comment) — an invalidation-contract decision this package deliberately leaves to the layer that wants the cache.
- Do not assume suspension is enforced on a host that never called `WithTenantStatusResolver`, whatever some other module's records say. The module is entirely additive and off by default; enforcement begins with the host's own wiring, and an unwired host's `Middleware` behaves byte-for-byte as before.
- Do not assume suspension is enforced on a host's allowlisted routes either, once the module IS wired. `WithAllowlist` waives the tenant-status gate's refusal exactly as it waives a resolution failure's, so an allowlisted route — an authentication entry that must work before a tenant is known, say — never enforces a suspension, however the status source answers; the gate's refusal is reserved for requests whose (method, path) the host did not allowlist, and a host that wants the refusal on a route must not allowlist that route, since no wiring of this module narrows the exemption.
- Do not include the `Resolver`'s own error in a response body. `Middleware` never does, because it may carry internal detail (an authenticated-request `Resolver`'s token-validation internals, for one).

**System context**

- Do not call `pkgcore.WithSystemContext` directly from code that is able to import `tenancy`. Use `tenancy.WithSystemContext` so the grant is audited. The raw primitive remains correct only for code at or below `tenancy` in the graph, which cannot import `tenancy` at all — plus two documented exceptions whose preconditions are stated in place at each call site: boot-time writes (a process-start declaration re-affirmed on every restart under a fixed actor has no operator session or ticket to attribute an audit row to, so no audit reader has a question its record would answer) and test-support code (an audit event per conformance-suite call is noise, not accountability). The escape hatch's uses split in two. Cross-tenant **widening** — seeing or acting beyond one tenant's own rows — stays restricted to `admin`/`compliance`/`jobs`/`authn`, and every grant of it goes through this audited wrapper. Platform-scope write **gates** are a different use, not a widening: a system-context-of-presence refusal gate on a platform-row write that operates strictly within its own scope, belonging to the module that owns the write path (`go/config`'s `ScopeSystem` tier, `ai-gateway`'s platform-credential write) and still going through this wrapper wherever it is importable. `dbkit.HardDelete` is the mechanism-level instance of the same gate: dbkit, below `tenancy`, checks presence only, and its grant holders are the four-module widening whitelist entering through this wrapper.
- Do not assume `WithSystemContext` widens what `dbkit.Repository[T]` can see or touch, in either direction, or that it substitutes for a tenant. `HardDelete` is the one `Repository[T]` method that consults the system context, and only to require it (refusing with `dbkit.ErrHardDeleteRequiresSystemContext` otherwise); nothing in `Repository[T]` widens or crosses tenant scope on account of a system context — see the cross-reference above and the tracked-debt entry in `docs/internal/17-risks.md`.
...
- Do not call `AssertIsolated` for identity or platform data, or `AssertNotTenantScoped` for tenant or link data. The data-domain classification decides which (tenant/link data → `AssertIsolated`, identity/platform data → `AssertNotTenantScoped`); getting it backwards either fails to compile or asserts the wrong property.
...
|---|---|---|
| `ErrTenantUnresolved` (`tenancy.tenant_unresolved`) | `Middleware`, when `resolver.Resolve` fails (or reports success with an empty tenant) on a (method, path) not covered by `WithAllowlist` | The 403 response is already written. Check the `Resolver`'s own source of truth (the custom-domain lookup for an unauthenticated request; the access-token claims for an authenticated one) |
| `ErrTenantSuspended` (`tenancy.tenant_suspended`) | `Middleware`, when a `WithTenantStatusResolver`-wired resolver reports the resolved tenant in any state other than `TenantStatusActive` — `TenantStatusSuspended` and any out-of-vocabulary answer with a nil error alike — on a (method, path) not covered by `WithAllowlist` | The 403 response is already written. Check the wired status source (a host's own tenant ledger); only a host that wired the module can ever see this error, and only for a tenant the source itself reports as non-active |
| `ErrTenantStatusUnavailable` (`tenancy.tenant_status_unavailable`) | `Middleware`, when a wired `TenantStatusResolver`'s `Status` call itself fails on a (method, path) not covered by `WithAllowlist` | Fails closed: the request is refused (500), never treated as "assume active". Fix whatever made the status source unreachable, then retry |
