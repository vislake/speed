...
- Do not cache a tenant's status between requests on top of this seam. `Middleware` consults the resolver synchronously on every successfully resolved request and caches nothing, so a suspension takes effect on the very next non-allowlisted request; a caching layer must first answer how a stale "active" answer is invalidated when a tenant is suspended (see `TenantStatusResolver`'s doc comment) — an invalidation-contract decision this package deliberately leaves to the layer that wants the cache.
- Do not assume suspension is enforced on a host that never called `WithTenantStatusResolver`, whatever some other module's records say. The seam is entirely additive and off by default; enforcement begins with the host's own wiring, and an unwired host's `Middleware` behaves byte-for-byte as before.
- Do not assume suspension is enforced on a host's allowlisted routes either, once the seam IS wired. `WithAllowlist` waives the tenant-status gate's refusal exactly as it waives a resolution failure's, so an allowlisted route — an authentication entry that must work before a tenant is known, say — never enforces a suspension, however the status source answers; the gate's refusal is reserved for requests whose (method, path) the host did not allowlist, and a host that wants the refusal on a route must not allowlist that route, since no wiring of this seam narrows the exemption.
...
| `ErrTenantSuspended` (`tenancy.tenant_suspended`) | `Middleware`, when a `WithTenantStatusResolver`-wired resolver reports the resolved tenant in any state other than `TenantStatusActive` — `TenantStatusSuspended` and any out-of-vocabulary answer with a nil error alike — on a (method, path) not covered by `WithAllowlist` | The 403 response is already written. Check the wired status source (a host's own tenant ledger); only a host that wired the seam can ever see this error, and only for a tenant the source itself reports as non-active |
