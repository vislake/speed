...
**The two dialect drivers are split into their own subpackages (issue #1's site 2).** `dbkit.Open`'s dialect selection is a small package-level registry mirroring `database/sql`'s own driver-registration pattern (`open.go`'s `RegisterDialect` / `dialectRegistry`) rather than a hardcoded `switch`. dbkit's root package no longer imports `github.com/glebarez/sqlite` or `gorm.io/driver/postgres` directly; each driver lives in its own subpackage, importable only for its `init` side effect: `github.com/vislake/speed/go/dbkit/dialect/sqlite` and `github.com/vislake/speed/go/dbkit/dialect/postgres`. A caller blank-imports the one(s) it needs, typically next to its `dbkit.Open` call site or in its module's main package:

```go
import _ "github.com/vislake/speed/go/dbkit/dialect/sqlite"
```

An unregistered dialect fails with the same `dbkit.invalid_dialect` code as before (no caller-visible error code changed), with a `hint` parameter naming the fix, mirroring Go's own "sql: unknown driver ... (forgotten import?)". `dbkit/dbtest` blank-imports both subpackages itself, so every existing `dbtest.NewSQLite`/`dbtest.NewPostgres` call site across the repo needed no change — bundling both drivers into a test-only package is correct and intentional, since test binaries are never a consumer's production dependency.

This does not shrink dbkit's own `go.mod`: both drivers stay direct requires there, because the dialect subpackages are part of the same module and import them directly. The measured win is at a *consumer's* `go.mod`, per the issue's own method (a throwaway module requiring only `github.com/vislake/speed/go/dbkit`, `go mod tidy` under `GOWORK=off`, counting `// indirect` entries): **41 indirect dependencies before this split, 27 after** — a drop of 14, covering both drivers' full transitive closures (`glebarez/go-sqlite`, `glebarez/sqlite`, `modernc.org/{libc,mathutil,memory,sqlite}`, `remyoudompheng/bigfft`, `mattn/go-isatty`, `golang.org/x/sync`, `gorm.io/driver/postgres`, `jackc/{pgpassfile,pgservicefile,pgx/v5,puddle/v2}`) — for a consumer that imports `dbkit` without blank-importing either dialect subpackage. `go/saasctl`'s own `go.mod` demonstrates this for real: it needs only `dialect/sqlite` (its `db migrate` command and generated skeletons never touch PostgreSQL), and tidying it after the split dropped `gorm.io/driver/postgres` and the whole `jackc/pgx` family from its indirect requires entirely.

**Out of scope.** Business queries beyond create / read-by-id / update / delete / list-all (`Repository[T]` is deliberately minimal — see Known limitations); the audited, business-facing wrapper around the system-context escape hatch (now `tenancy.WithSystemContext`); provisioning the PostgreSQL roles and RLS policies isolation layer 3 depends on, which is a deployment-side responsibility this package assumes but does not create; the whitelist that decides which modules may hold a system context at all, and the audited entry through which one is granted — the caller-side obligations of `HardDelete`'s gate, enforced outside this package (see "Hard deletion" below); retention-window enforcement for `Restore`, which is `compliance` module (M4) territory that does not exist yet.
...
| The unexported `softDeleteScopePlugin`, installed automatically by `Open` on every connection it returns, alongside `tenantScopePlugin` | For any model implementing `SoftDeletable`, auto-appends `deleted_at IS NULL` ahead of every **query** callback only (`Before("gorm:query")`) — deliberately narrower than `tenantScopePlugin`, which also covers create/update/delete. `db.Unscoped()` (GORM's own general bypass) skips the filter entirely, for an admin-style "see soft-deleted rows" read path. A model implementing neither `TenantScoped` nor `SoftDeletable` is untouched by either plugin |

**What "soft delete" means here, precisely** (docs/internal/04-data-and-tenancy.md's delete-semantics section — mark-delete/recoverable vs. hard-delete/compliance-erasure): `Repository[T].Delete` branches on `T`'s own capability. A `T` implementing `SoftDeletable` gets one `UPDATE ... SET deleted_at = ?, deleted_by = ?` — the row stays in the table, hidden from ordinary reads only by `softDeleteScopePlugin`'s query-time filter. Every other `T` keeps today's real, physical `DELETE`, byte-for-byte — this is an **additive, per-model opt-in capability**, never a new implicit default, and never a breaking change to `Delete`'s existing signature or meaning for a non-`SoftDeletable` model. `Restore` is the mark-delete's inverse, clearing both columns.
...
**The unique-index interaction is a per-model decision — this package does not force one answer.** A soft-deleted row still occupies whatever unique constraint it had while live (`go/org`'s `UNIQUE(tenant_id, parent_id, name)` is the design doc's own named example). This round's proof, `testutil.SoftDeletableWidgetTableSQL`, adopts a **partial unique index** — `CREATE UNIQUE INDEX ... ON <table> (tenant_id, <col>) WHERE deleted_at IS NULL` — as the answer for that fixture: standard SQL, not PostgreSQL-specific, proven dual-dialect-safe by running the identical DDL string against both SQLite (`soft_delete_unique_index_test.go`) and real PostgreSQL (the RLS integration test above). It lets a name be reused immediately after its holder is soft-deleted, while a duplicate *live* name is still rejected, and the index stays correctly tenant-scoped via its leftmost column. A model that instead wants "no reuse until the row is actually cleaned up" keeps its ordinary, non-partial unique index — also legitimate; this package takes no position on which a given model should pick, only that the choice (per the design doc's own instruction) must be made explicitly, not silently skipped.

**Retention windows are not enforced.** The design doc frames `Restore` as working "within the retention window" — but retention-window *configuration* is `compliance` module (M4) territory that does not exist yet. `Restore`, as implemented today, succeeds unconditionally for any currently-soft-deleted row under the caller's tenant, with no deadline. A future round wiring real retention needs to add that check here (or in a caller above this package); do not assume it already exists because the design doc mentions it.

**`Update` does not respect `deleted_at`, and this is a real, documented, un-fixed hazard — not an oversight to silently work around.** `Repository[T].Update`'s existing full-record `Select("*")` save writes every column, `deleted_at`/`deleted_by` included, and the soft-delete auto-scope is query-only (see the plugin table above) — `Update` was deliberately left untouched by the round that added `SoftDeletable`/`Restore`. The result: calling `Update` with a stale in-memory `*T` captured *before* someone else's `Delete` silently "undeletes" the row, since the stale copy's `DeletedAt`/`DeletedBy` are the zero values and `Update` writes them back unconditionally. `TestRepository_Update_StaleModelAfterSoftDelete_SilentlyClearsDeletedAt` (`repository_test.go`) pins this exact, current behavior — a passing test there means the hazard still exists, not that it is fixed. Extending the auto-scope to cover `Update` is a real option for a future round, once a caller actually needs it (see the codebase's stated philosophy against speculative generality); it is not done today.

### Hard deletion — `hard_delete.go`

`Repository[T].HardDelete(ctx, id)` is the restricted, irreversible half of the delete semantics — the sequential-next phase docs/internal/04-data-and-tenancy.md's delete-semantics section (§3) frames as mark-delete's follow-up, landed in the round after `SoftDeletable`/`Restore`. It physically removes the row with the given id: a real `DELETE`, never a mark-delete, and for a `SoftDeletable` `T` a soft-deleted row is just as removable as a live one — `softDeleteScopePlugin` is query-only, so nothing turns the DELETE into a no-op, and `HardDelete` never consults `SoftDeletable` at all. A `T` that never implemented `SoftDeletable` (whose ordinary `Delete` was always physical) gets exactly the semantics `Delete` already had, now behind the gate. Body-wise, `HardDelete` is deliberately byte-identical to `Delete`'s physical branch after the gate — same tenant resolution, same `WithTenantSession` transaction, same `ErrRecordNotFound` on zero rows affected — per the per-file duplication convention `hard_delete.go`'s own doc comment records. No migration is involved: it issues the physical `DELETE` the schema always permitted, which is why the mark-delete round's migration files say they add nothing for `HardDelete`.
...
**Known limitation — do not wire `AuditBus` to the same SQLite file a synchronous, same-process persister writes into.** `dbkit.Repository[T]`'s writes run inside `WithTenantSession`'s transaction, and `auditCapturePlugin`'s `After("gorm:create"/"gorm:update"/"gorm:delete")` callback fires — and publishes, synchronously, on the standalone in-memory `EventBus` — *while that transaction is still open*. If the subscriber on the other end of that publish (typically `audit.Module`, see below) inserts into a database on the **same SQLite file**, from the **same goroutine**, it opens a second write session against a file that already holds an uncommitted write lock: SQLite allows only one writer per file, so this deadlocks into `database is locked` (`SQLITE_BUSY`) on every single captured write, not merely under contention — confirmed empirically wiring `examples/reference-app` for the M1 audit-infrastructure round's B3 block (see that round's report and `examples/reference-app/cmd/server/server.go`'s own doc comment on its `dbkit.Open` call). Raising `busy_timeout` does not help: the same goroutine holds the lock it would be waiting to clear, so it is a self-deadlock, not ordinary contention. PostgreSQL's finer-grained, MVCC locking makes this far less likely to reproduce identically, but sharing one transaction-bearing connection between an audited write and its own synchronous persister remains an unnecessary coupling there too, and has not been proven safe by this round's tests.
Until a future round fixes the mechanism itself (most likely by deferring the plugin's publish until after the enclosing transaction actually commits, rather than from inside the GORM callback chain), a host wiring `AuditBus` should either point the persister (`audit.Module`) at a genuinely separate database/connection from the audited model's own, or avoid the automatic mechanism for models whose writes go through `dbkit.Repository[T]` and use `audit.Emit` explicitly instead, called *after* the triggering write has returned (so after its own transaction has committed) — see `examples/reference-app/internal/notes/handler.go`'s `recordNoteCreatedAudit` for a real, tested example of the latter.
...
**Soft deletion**
- Do not extend the `deleted_at` auto-scope (`softDeleteScopePlugin`) to `Update` or `Delete` without checking whether `Restore`'s defensive `.Unscoped()` call still holds afterward. The plugin is deliberately query-callback-only today (see the "Soft deletion" section above); `Delete`/`Restore` build their own explicit `deleted_at IS NULL` / `IS NOT NULL` conditions instead of relying on it, and `Restore`'s `.Unscoped()` is currently a no-op kept for exactly this future possibility — broadening the scope without re-checking that interaction could silently make `Restore` unable to reach the very row it means to clear.
