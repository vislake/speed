...
| `type TenantModel struct { TenantID string }` — tag `` `gorm:"column:tenant_id;not null;index:,priority:1"` `` | Embeddable base satisfying `TenantScoped` and giving the plugin its known column. Note: the tag does **not** include `primaryKey` (see Known limitations); none of dbkit's own fixtures currently embed it — `internal/testutil.Widget` declares `TenantID` directly instead, with its own `primaryKey` tag |
...
- `tenant_id` is the leftmost column of every composite index, and a tenant-scoped table's primary key should be `(tenant_id, id)`. `TenantModel`'s own tag does not declare `primaryKey` — declare `TenantID` directly with the tag you need (as `internal/testutil.Widget` does) if the composite key matters to you.
...
This example is illustrative; dbkit does not yet ship an `example_test.go` compiling it under CI (see the Documentation rule below). `repository_test.go`'s own `TestRepository_FullCRUDLifecycle_SingleTenant` exercises the identical call sequence — `Create` → `FindByID` → `Update` → `Delete` — against `internal/testutil.Widget`, and is the closest thing to a verified reference today.
...
- Do not assume `TenantModel` alone gives you the standard's recommended `(tenant_id, id)` composite primary key — its tag does not include `primaryKey`.
...
**Documentation**
- Do not add an exported identifier to this package without a doc comment and an entry in the tables above, in the same pull request (backend coding standard, Documentation section) — and, once dbkit's own `example_test.go` exists, a compiling `Example`.
