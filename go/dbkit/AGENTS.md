...
**Out of scope.** Business queries beyond create / read-by-id / update / delete / list-all (`Repository[T]` is deliberately minimal — see Known limitations); the audited, business-facing wrapper around the system-context escape hatch (now `tenancy.WithSystemContext`); provisioning the PostgreSQL roles and RLS policies isolation layer 3 depends on, which is a deployment-side responsibility this package assumes but does not create; `HardDelete` — the irreversible, system-context-gated physical-erasure path docs/internal/04-data-and-tenancy.md's delete-semantics section (§3) describes as mark-delete's sequential-next phase, deliberately not started or stubbed in this package as of this writing (see "Soft deletion" below); retention-window enforcement for `Restore`, which is `compliance` module (M4) territory that does not exist yet.
...
- Do not treat a soft-deleted row as compliance-grade deletion, and do not write documentation, error messages, or product copy that implies otherwise. See the "Soft deletion" section's own warning; `HardDelete` (not implemented) is what that actually requires.
...
| `ErrRecordNotFound` (`dbkit.record_not_found`) | `Repository[T].FindByID` / `Update` / `Delete` / `Restore` when nothing matches both the id and the caller's tenant (for `Restore`, additionally requiring a currently-soft-deleted row) | Ordinary not-found handling. Deliberately indistinguishable from "that id belongs to another tenant" (and, for `Restore`, from "exists but was never deleted") |
