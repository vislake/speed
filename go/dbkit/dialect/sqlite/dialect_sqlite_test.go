...
// The busy-timeout tests below pin the busy_timeout contract
// withDefaultPragmas declares (dialect_sqlite.go): every connection
// dbkit.Open returns carries the bounded default, ordinary writer
// contention waits through it and converges once the holder commits, a
// hold that outlasts it fails with the real busy error instead of
// hanging, and -- the boundary spelled out in withDefaultPragmas' doc
// comment -- SQLite's deadlock avoidance answers a read-then-write
// upgrade with an immediate busy error that no timeout setting waits
// out.
//
// These are contract-pinning tests, deliberately not regression tests:
// the semantics they pin are supplied today by two independent
...
// effect), exactly the shape a two-worker race in a standalone deployment
// has. Lock hold and release are sequenced by the test itself -- the
// holder commits only after the contender has reported, or after a fixed
// grace window -- never by sleeping until some assumed interleaving
// happens, so there is no sleep race in any direction: each test's
// outcome is determined by SQLite's lock protocol plus one generous grace
// window, not by which goroutine the scheduler happened to run first.
