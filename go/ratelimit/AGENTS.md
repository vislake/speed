...
ratelimit is speed's shared rate-limiting primitive: a `Limiter` that decides whether one more hit against a caller-supplied `key` is within a caller-supplied `Limit`, backed entirely by `pkgcore.KVStore`. It sits at the same graph depth as `dbkit`, `observability` and `tenancy` — directly above `pkgcore`, depending on nothing else (`pkgcore -> dbkit / observability / ratelimit -> tenancy -> ...`, see `docs/internal/01-architecture.md`) — and is a **pure library**: unlike every other module in this repository, it does not implement `the module contract`. It registers no routes, no config schema, no feature flags, no permissions. There is nothing to wire into a Kernel; a consumer just calls `ratelimit.New`.
...
**No dynamic configuration.** `Limit` values are supplied by each call site directly in code; no schema-backed, operator-tunable settings exist here. This is deliberate for the same reason ratelimit implements no `the module contract`: it is a library, not a component the Kernel assembles.
...
- Do not make ratelimit implement `the module contract`. It is intentionally a pure library with nothing to register.
