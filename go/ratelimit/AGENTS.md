...
ratelimit is speed's shared rate-limiting primitive: a `Limiter` that decides whether one more hit against a caller-supplied `key` is within a caller-supplied `Limit`, backed entirely by `pkgcore.KVStore`. It sits at the same graph depth as `dbkit`, `observability` and `tenancy` — directly above `pkgcore`, depending on nothing else (`pkgcore -> dbkit / observability / ratelimit -> tenancy -> ...`, see `docs/internal/01-architecture.md`) — and is a **pure library**: unlike every other module in this repository, it carries no module contract. It registers no routes, no config schema, no feature flags, no permissions. There is nothing to select in a composition; a consumer just calls `ratelimit.New`.
...
**No dynamic configuration.** `Limit` values are supplied by each call site directly in code; no schema-backed, operator-tunable settings exist here. This is deliberate for the same reason ratelimit carries no module contract: it is a library, not a component the assembly drives.
...
- Do not give ratelimit a module contract. It is intentionally a pure library with nothing to register.
