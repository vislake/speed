...
// 0008_single_root_live) and rbac two
// (0001_create_rbac, 0002_add_soft_delete) -- 21 in total, tables configs,
...
func TestMigrateFreshDatabaseAppliesTheWholeRequiredUniverse(t *testing.T) {
	...
	want := fmt.Sprintf("Migrated %s: applied 21 migration files (authn 10, config 1, org 8, rbac 2)\n", dbPath)
	if stdout != want {
...
func TestMigrate_PKIRequired_MigratesPKITablesToo(t *testing.T) {
	...
	want := fmt.Sprintf("Migrated %s: applied 30 migration files (authn 10, config 1, org 8, pki 9, rbac 2)\n", dbPath)
	if stdout != want {
		t.Errorf("stdout = %q, want %q", stdout, want)
	}

	gdb := openDB(t, dbPath)
	wantLedger := map[string]int{"authn": 10, "config": 1, "org": 8, "pki": 9, "rbac": 2}
...
func TestMigrateRerunOverTheSameDatabaseReportsUpToDate(t *testing.T) {
	...
	want := fmt.Sprintf("%s is up to date: schema_migrations already records 21 migration files (authn 10, config 1, org 8, rbac 2)\n", dbPath)
	if stdout != want {
		t.Errorf("stdout = %q, want %q", stdout, want)
	}
...
func TestMigrateAppliesOnlyWhatAGrowingGoModNewlyRequires(t *testing.T) {
	...
	want = fmt.Sprintf("Migrated %s: applied 20 migration files (authn 10, org 8, rbac 2)\n", dbPath)
	if stdout != want {
		t.Errorf("stdout = %q, want %q", stdout, want)
	}
	if got := ledgerCounts(t, openDB(t, dbPath)); !reflect.DeepEqual(got, fullUniverseLedger) {
