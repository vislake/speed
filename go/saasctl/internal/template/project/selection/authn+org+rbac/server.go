...
// mounted route is admitted through the route table below it, so a module
// added to the --with set later cannot start up with an undecided route.
//
...
func (b *serverBuild) cryptoComponent() pkgcore.Component {
	...
	return pkgcore.Component{
		...
		Prepare: func(_ context.Context, reg *pkgcore.ComponentRegistry) error {
			...
			// org's Invitation.Email column is encrypted at rest under the
			// platform cipher and made queryable by a SEPARATE HMAC key the
			// module's own descriptor reads; reusing the cipher key for both
			// would be exactly the AES-key-doubling-as-an-HMAC-key weakness
			// dbkit warns about. The module's own registrar owns the
			// serializer name, so it never crosses this wiring as a
			// hand-typed string.
...
// refuses the boot unless the composition disabled it. An override owns
// its whole construction: the module's construction-time component block
// and its optional seams are not applied through this path.
func (b *serverBuild) orgComponent(reg *pkgcore.ComponentRegistry) (pkgcore.Component, error) {
	return overrideComponent(reg, "org", "org", func(_ context.Context, reg *pkgcore.ComponentRegistry, _ pkgcore.ComponentConfig) (any, error) {
		...
		indexer, err := org.NewEmailIndexer(indexKey)
		if err != nil {
...
func (b *serverBuild) rbacComponent(reg *pkgcore.ComponentRegistry) (pkgcore.Component, error) {
	return registerOnlyComponent(reg, "rbac", func(instance any, reg *pkgcore.ComponentRegistry) error {
		...
		if !ok {
			return fmt.Errorf("__APP_NAME__: the rbac component was handed a %T instance, want *rbac.Module", instance)
		}
		return m.Register(reg)
	})
}
...
// that runs after every module's own Init turn has declared, takes the two
// freeze points those declarations feed -- config's schema snapshot and
// rbac's permission catalog -- through the modules' own Attach calls,
// publishes both services into the registry for the steps and the face that
// follow, and publishes the merged message catalog.
func (b *serverBuild) postBootstrapComponent() pkgcore.Component {
	return pkgcore.Component{
		...
		Init: func(_ context.Context, reg *pkgcore.ComponentRegistry, _ any) error {
			...
			// permission declared after the snapshot could never be granted
			// at all.
			if b.rbacService, attachErr = b.rbacModule.Attach(reg); attachErr != nil {
				return fmt.Errorf("__APP_NAME__: attach the rbac module: %w", attachErr)
			}
			reg.Put(b.rbacService)
			return nil
		},
	}
}
...
func (b *serverBuild) bindRegistry(reg *pkgcore.ComponentRegistry) error {
	...
	if b.orgModule, err = pkgcore.Get[*org.Module](reg); err != nil {
		return fmt.Errorf("__APP_NAME__: read the org module: %w", err)
	}
	...
	if b.rbacModule, err = pkgcore.Get[*rbac.Module](reg); err != nil {
		return fmt.Errorf("__APP_NAME__: read the rbac module: %w", err)
	}
	return nil
}
...
// then the protected face composeFace builds over it.
...
// below), through which rbac.GuardRoutes admits every mounted route, wraps
// each gated one in rbac's fail-closed gate before it is mounted, and fails
// -- so this assembly fails -- when a mounted path has no declared decision
// or the table names a path no module mounted.
