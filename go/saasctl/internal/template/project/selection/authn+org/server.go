...
func buildServer(ctx context.Context, cfg serverConfig) (http.Handler, func() error, error) {
	...
	for _, m := range []pkgcore.Module{authnModule, orgModule, configModule} {
	...
	reg, err := pkgcore.NewKernel(kernelOptions...).Bootstrap(ctx, authnModule, orgModule, configModule)
	if err != nil {
		_ = cleanup()
