...
// the step that freezes it. The middleware chain is speedchain.Chain's composition: authn
// first, so each token is verified exactly once and the tenant comes from
// the verified Principal's claims, never a Host header, then tenancy
// under the chain's pre-auth allowlist -- authn.Middleware is optional
// auth (a bad token is a 401, an absent one stays anonymous), so
...
// all -- speedchain.Chain dispatches the whole subtree under
// speedapp.AuthnAPIPath ahead of tenancy (see mountModuleRoutes), which
// is also what lets enterprise SSO's dynamic per-tenant provider names
