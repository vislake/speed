...
// the modules the WithModules stage constructs, the service the
// post-bootstrap attach derives from them, and the Redis resources this host
// built and therefore owns. The declared key materials are not carried here:
// the assembly resolves them off the module components' declarations and
// hands them to the callbacks as the published bootstrap material.
...
//
// This composition wires the authn, org and config modules (the generator's
...
// Register) lands before the step that freezes it.
func (b *serverBuild) constructModules(_ context.Context, deps speedapp.ModuleDeps) ([]pkgcore.Module, error) {
	...
	return []pkgcore.Module{b.pkiModule, b.authnModule, b.orgModule, b.configModule}, nil
}
