...
// moduleName is notification's module name, the value Name() answers. It is
// also the key dbkit.MigrationRegistry.Register builds its dependency graph
// on.
...
// Module carries the module contract for go/notification: the tenant's
...
// assembly's Init stage. The module's tables live in db, which the host
// opened through dbkit.Open (so it carries the tenant-isolation plugin) and
// migrated before the assembly runs. Register then validates the module's
// required seams, contributes the module's declarations -- its event
// catalog, its audit actions, its job handler -- to the host's registry,
// and attaches the registrars and the host registry the services need. The
// module performs no I/O of its own during registration.
type Module struct {
	...
	// The inbox stream's own connections subscribe scoped through
	// SubscribeFor (hub.go's doc comment); the delivery pipeline is the
	// producer side.
	hub *Hub
...
// implementation (pkgcore/sms_console.go); a distributed host may hand
// over any
...
// cipher -- see contact.go's doc comment for why the two stay separate.
...
// DependsOn implements the module contract: nothing. notification sits
// above jobs, config and dbkit in the module dependency graph, but none of
// those carries a module contract -- they are libraries the host wires, and
// DependsOn enumerates only modules in the assembly's component set.
// notification must also NOT
...
// Locales implements the module contract: the descriptions of notification's
...
// OpenAPISpec implements the module contract: it returns notification's own
...
// handler.go), so the spec and its implementation cannot drift. The one
// route this module serves that the fragment deliberately does not carry,
// the inbox stream, is documented in the fragment's own header and mounted
// by handler.go.
...
// it only declares and wires -- no database call, no outbound call, nothing
// that touches m.db.
//
