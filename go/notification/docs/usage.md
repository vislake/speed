...
| `WithMailFrom(addr)` | The `From:` address of every email this module composes | A plain string; the module has no mail-provider config of its own -- it sends through the `pkgcore.Mailer` seam the host's assembly resolves. |
