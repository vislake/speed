...
| The `org.member.removed` subscriber and its JSON-shaped payload probe — the reap of a removed member's orphaned role bindings (see "Soft deletion") | `reap.go` |
...
| D13 | `RoleBinding.Restore` re-validating against `AssignRole`'s own preconditions | `RestoreRole` changes nothing about the restored row but its two soft-delete columns, mirroring `org.Membership.Restore`'s identical documented choice — see "Soft deletion" | a round that wants stricter Restore semantics |
