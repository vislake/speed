...
-- the full rationale of every column and index. The two files are kept
-- identical in substance on purpose, following the dual-dialect
-- constraints every module's migrations follow: no dialect-specific types,
-- no gen_random_uuid(), no NOW(), no native arrays, no JSONB.
--
