...
// SQLite unit leg of the P0-pki-6 regression (a): a database whose
// revocation ledger holds two rows for one certificate -- the duplicate
// shape round 3's pre-arbitration RevokeCertificate could leave behind --
// must upgrade through the module's migration set and be left with exactly
// one row per certificate. This test fails against the pre-fix 0008 file:
// its CREATE UNIQUE INDEX refuses to build over the seeded duplicates, and
// the module transaction rolls back, stranding the upgrade exactly as the
// finding describes. The PostgreSQL leg runs the same scenario against a
// real server in go/pki/integration_test/postgres_migration_dedupe_test.go.
...
// is the SQLite unit leg of the P0-pki-6 regression (b): a database that
// already applied 0008 is unaffected by a later migration run -- the
