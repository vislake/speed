...
-- shape a check-then-act Create cannot provide.
--
...
-- certificate_id, and the ledger as 0007 defined it carries no such
-- guarantee: an unconditional Create behind a check-then-act
-- certificate-status read lets two callers revoking one active
-- certificate concurrently both pass the check and land TWO rows for
-- it -- a state 0007's non-unique per-certificate index could not
-- refuse. Such a database cannot build the unique index
...
-- (isApplied/recordApplied in go/dbkit/migrations.go). A database stranded
-- at 0008 -- the failure rolled its transaction back, leaving no record
-- behind -- reaches this file on its next startup and is rescued; a
-- database that already applied this file is untouched, so no follow-up
-- migration is needed for either class.
--
