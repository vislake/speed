...
// added in the round that fixed the module's three PostgreSQL-only
// defects. It is physically separate from go/metering's unit tests (which
...
//
// What this tier exists to prove, per the three defects it was built
// against (each with its own regression test below, plus the
// isolation-suite re-runs in this file):
//
//   - outbox.go's Enqueue recovered a unique-key conflict by reading the
//     existing row back on the SAME transaction. SQLite tolerates that;
//     PostgreSQL does not: after a statement error the transaction is
//     aborted and every later statement fails with SQLSTATE 25P02 until
//     ROLLBACK, so the read-back failed, Enqueue returned the insert
//     error, and the idempotent-retry contract collapsed -- a caller's
//     retry of an Enqueue whose first attempt already committed got an
//     error and rolled back its own unrelated business write along with
//     it. The recovery now inserts with ON CONFLICT DO NOTHING, which
//     never aborts the transaction, and only then reads the pre-existing
//     row back on the still-healthy transaction
//     (postgres_outbox_test.go's TestPostgres_Enqueue_
//     IdempotentRetry_InsideOneCallerTransaction, which fails against
//     the pre-fix code on real PostgreSQL and passes after).
//
//   - aggregator.go's foldIntoSummaryOnce swallowed the receipt-insert
//     unique violation and returned nil, so the enclosing
//     dbkit.WithTenantSession transaction committed. On PostgreSQL a
//     COMMIT issued against an aborted transaction is turned into a
//     ROLLBACK by the server, which the pgx driver surfaces as an error
//     ("commit unexpectedly resulted in rollback") -- so a redelivered
//     event made IngestBillingGrade fail, Dispatcher left the outbox row
//     pending forever, and the receipt's "exactly once" held only on
//     SQLite. The receipt insert now also uses ON CONFLICT DO NOTHING:
//     the conflict is recognized through RowsAffected == 0 with the
//     transaction healthy, and the commit is a real commit
//     (postgres_ingest_receipts_test.go's two regressions, both of
//     which fail pre-fix on real PostgreSQL).
//
//   - recorder.go's UsageEvent.validate bounded only Metadata. TenantID,
...
//     same aborted-transaction knock-on as the first defect. validate now
...
//     which fails pre-fix on real PostgreSQL).
//
...
// tenant data, per docs/internal/04-data-and-tenancy.md) against the real
// server, the same re-run go/org, go/rbac, go/storage and go/notification
// integration tiers make for their own tenant-scoped repositories.
...
// metering_usage_summaries is tenant data (docs/internal/
// 04-data-and-tenancy.md); AssertIsolated is the mandatory suite for it.
