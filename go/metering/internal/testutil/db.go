...
// per-call, temp-file database from dbtest.NewSQLite (so it carries
// dbkit's full wiring, isolation plugin included), with fs's sqlite/*.sql
// applied from zero through the real dbkit.MigrationRegistry.
//
// Applying the module's actual migration files, rather than an
// AutoMigrate or a hand-written CREATE TABLE, is what makes every test
// that uses this helper also a proof that those files run from zero -- the
// property the pre-commit checklist asks for and that AutoMigrate is
// banned for not providing.
...
// calls it yet -- see go/metering/AGENTS.md's Known limitations -- but the
// helper is included now so a later round's PostgreSQL leg needs no db.go
// of its own, mirroring go/pki/internal/testutil's identical choice.
...
// needs in order to apply one module's migration files. It carries
// nothing but a name and the embedded files, which is exactly why this
// package does not have to import the module whose migrations it is
// applying.
