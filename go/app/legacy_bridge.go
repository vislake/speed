...
// stays the module Registry's own registrar: the component model has no
// bootstrap-key seat (components declare BootstrapKeys statically), so a
// legacy module's bootstrap declarations collect there for the host's
// binding verification, exactly where they collected before.
...
func (s assetStandIn) Register(*pkgcore.Registry) error { return nil }
...
// registry view the wrapped modules declare into; each wrapper's Init calls
// exactly the module's Register, the module's one declaration entry point.
func wrapLegacyModules(modules []pkgcore.Module, wrapped *pkgcore.Registry) ([]pkgcore.Component, error) {
	...
	for _, m := range modules {
		...
		components = append(components, pkgcore.Component{
			...
			Init: func(_ context.Context, _ *pkgcore.ComponentRegistry, _ any) error {
				return module.Register(wrapped)
			},
		})
	}
