...
// declarations without subscribing to any of them. Register declares both up front, so
// the catalog is already complete for the subscribers that will listen
// for them: ObjectService.Complete publishes EventObjectCompleted, and
// LifecycleService publishes EventObjectDeleted.
...
// Module carries the module contract for go/storage: object metadata and its
...
// is refused unless the host states at construction, out in the open, that
// its deployment allows permanent objects. A host
...
//
// This is a real answer, not a stub. storage's dependence on a queue is a
// seam the host wires (WithQueue), not a requirement that the jobs module
// itself be in the assembly's component set -- DependsOn enumerates only
...
// Locales implements the module contract: the descriptions of storage's
...
// OpenAPISpec implements the module contract: storage's own OpenAPI fragment,
...
// the spec-first decision that the fragment is the single source of the
...
func (m *Module) Register(reg *pkgcore.ComponentRegistry) error {
	...
	// gate keeps the three facts -- which types have a decoder, which have
	// a metadata strip, what the whitelist may admit -- in one checked
	// place (validate.go's checkAdmittedMediaType).
