...
-- JSONB, no gen_random_uuid(), no NOW(). id is an application-generated
-- UUID (the backend coding standard bans gen_random_uuid(), which SQLite
-- has no equivalent for), and created_at / updated_at are written by
-- gorm's autoCreateTime and autoUpdateTime, never by a database default.
--
