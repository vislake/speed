...
-- live token at a time") with a real database constraint: without it, the
-- revoke-then-insert shape of an Invite call leaves a window in which two
-- concurrent InviteService.Invite calls for the SAME
-- address can both observe no pending invitation to revoke and both
-- INSERT, landing two simultaneously live tokens for one address --
-- something the module's own doc comment claims can never happen.
--
...
-- inserts the new row (in the same transaction -- see
