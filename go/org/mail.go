...
// invitationDefaultLocale is the platform default language: the terminal
// tier of the invitation locale chain (invite.go's invitationLocale), and
// the same en-US default the web side's createI18n falls back to last.
//
// Falling back to it rather than failing is right because every input the
// chain consumes is a preference, not a command -- a requester may name a
// locale the platform carries no catalog for, or name none at all -- and
// an invitation that cannot pick a language must still go out. Falling
// back at LOOKUP time would be wrong, and the catalog refuses to: a locale
// it serves must carry every id.
const invitationDefaultLocale = i18n.LocaleENUS
