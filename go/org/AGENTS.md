...
`Repository` embeds `*dbkit.Repository[OrgNode]` (Create / FindByID / Update / Delete / Restore / List promoted unchanged — `Delete`/`Restore` are mark-delete/undo since the "Soft deletion" round, because `OrgNode` implements `dbkit.SoftDeletable`) and adds the query shapes `Repository[T]`'s minimal surface cannot express: `subtree`, `children`, `findRoot`, `bySiblingName`, `byIDs`, `deleteLeaf`, `deleteSubtree` — all unexported, all reached through `TreeService`.
