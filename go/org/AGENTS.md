...
`Repository` embeds `*dbkit.Repository[OrgNode]` (Create / FindByID / Update / Delete / Restore / List promoted unchanged — `Delete`/`Restore` are mark-delete/undo since the "Soft deletion" round, because `OrgNode` implements `dbkit.SoftDeletable`) and adds the query shapes `Repository[T]`'s minimal surface cannot express: `subtree`, `children`, `findRoot`, `bySiblingName`, `byIDs`, `deleteLeaf`, `deleteSubtree` — all unexported, all reached through `TreeService` — plus the concurrency-hardening round's own primitives, `touchLockByID` and `lockLiveNode` (`assertNameFreeTx` is the sibling-name pre-check's transaction-bound twin). See "Concurrency" below for what those three exist to close.
...
**Proof, and its honest limits.** `MemberService.Remove` and `InviteService.Invite`'s races are reliably reproducible with two real goroutines racing a real, file-backed SQLite database (`membership_test.go`'s `TestMemberService_Remove_ConcurrentLastTwoActiveMembers`, `invite_test.go`'s `TestInviteService_Invite_ConcurrentSameAddress_ExactlyOneLiveToken` — each repeats the pair `trials` times since any single pair's own reproduction rate on the pre-fix shape was measured around one in five, not one in one). `CreateChild`/`Move`/`Restore`'s races sit between two DIFFERENT transactions' single statements, which no test can deterministically pause between without a test-only hook this round does not add to production code; `tree_test.go`'s five `TestTreeService_Concurrent*` tests instead run many rounds of two real operations racing and re-check the tree's structural invariant (`assertNoOrphans`: every live node's `ParentID` names another live node, and its `Path` is exactly that parent's plus its own id) after every round — proven RED against the pre-fix tree.go/repository.go for four of the five pairings tested (`CreateChild`×`Delete`, `Move`×`Move`, `Move`×`CreateChild`, `Restore`×cascade-`Delete`; `Move`×`Delete` did not reproduce within these tests' own round counts, though the module-level fix is identical). `integration_test/postgres_concurrency_test.go` re-runs the same five pairings against a real PostgreSQL server — the tier that actually caught the `deleteLeaf`/`deleteSubtree` gap above, which the SQLite unit tier's coarser whole-file locking could not expose.
...
`internal/testutil.NewPostgres` is the integration tier's counterpart, applying the identical files from `postgres/`, so the same assertions can run against both dialects by swapping the constructor alone. That tier is `integration_test/` (`//go:build integration`, `go test -race -tags=integration ./...` — Docker required): `postgres_tree_test.go` re-runs the A1 dialect-identity proof (the sibling-sharing-a-prefix subtree scan, a whole-subtree `Move`, a cascading delete, the max-depth bound) against a real PostgreSQL, `postgres_isolation_test.go` re-runs `tenancytest.AssertIsolated` for all three tenant-scoped repositories, including the encrypted-and-blind-indexed `Invitation.Email` column, and `postgres_concurrency_test.go` (the concurrency-hardening round) re-runs D1/D2/D5's five racing-operation pairings against a real server — see "Concurrency" above for why this tier, specifically, is what caught the `deleteLeaf`/`deleteSubtree` gap that round's own fix needed. It is wired into CI on `full-check.yml`'s `integration-tiers` matrix and into `Taskfile.yml`'s `INTEGRATION_DIRS`.
