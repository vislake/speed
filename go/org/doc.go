...
//
//   - Users. The users table is identity data and belongs to authn. org
//     never creates, reads or imports
...
//     caller's authenticated subject. That is the canonical shape of the
//     module-boundary rule -- authn publishes
...
//     module.
//
//   - Roles and permissions. The design sketches a Roles field on
//     Membership; org does not store one. Role
//     state is rbac's policy store, keyed by tenant, user and node
//     path -- and a []string column has no portable dual-dialect
//     representation anyway (native arrays are not portable across the
//     two dialects). org supplies the tree and the
