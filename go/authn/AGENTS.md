...
The component's `ConfigSchema` declares two process-start keys as `derive` fields -- `pii_cipher_key` and `blind_index_key` under the component's `"authn"` namespace, so they resolve at the exact platform key paths the module exports as `PIICipherKeyPath`/`BlindIndexKeyPath`: `authn.pii_cipher_key` (the AES key behind `RegisterPIISerializer`'s field serializer) and `authn.blind_index_key` (the HMAC key behind `WithBlindIndexKey`). Both are `[]byte` and Sensitive, with `ConfigDocs` contract text stating a non-secret development default, and both are separate secrets on purpose -- an AES key never doubles as an HMAC key, and the rule spans modules, not only authn's own two keys. The declaration states the module's contract; the assembly's loader resolves each key through the five-source chain (flags, environment, an optional file, the root-key derivation, the declared defaults) and publishes the material at the same key path, which `Prepare` (the PII serializer, which must precede the connection that parses the models) and `New` (the blind indexer) read by path. The module never reads the environment itself. `pkgcore/AGENTS.md` carries the contract of both declaration faces, including why key material on both the process-start and the runtime configuration layers is refused.
