...
| The frontend half of this module's API contract — the generated authn surface of `@speed/api-sdk` — has no runtime end-to-end consumer yet. | The generated half itself is live since the auth-core round: the authn fragment joins notes' in the merged `build/openapi/speed.yaml` (redocly), and pinned orval regenerates the authn surface into `@speed/api-sdk`, compile-consumed in-workspace by `@speed/auth-core` (its suite type-checks and exercises the operations through the scripted `bindRequestFn` harness — the same seam a host's real client binds). What remains is the runtime consumer: reference-app shells driving real sign-ins, landing with the `auth-ui` round. |
