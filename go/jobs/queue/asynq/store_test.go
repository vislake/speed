...
func TestQueueForPriority(t *testing.T) {
	tests := []struct {
		...
		{name: "low", p: jobs.PriorityLow, want: queueLow},
		{name: "normal", p: jobs.PriorityNormal, want: queueDefault},
		{name: "high", p: jobs.PriorityHigh, want: queueCritical},
		{name: "below low collapses to low", p: jobs.Priority(-5), want: queueLow},
		{name: "above high collapses to critical", p: jobs.Priority(100), want: queueCritical},
		{name: "between low and normal collapses to default", p: jobs.Priority(2), want: queueDefault},
		{name: "between normal and high collapses to default", p: jobs.Priority(7), want: queueDefault},
	}
...
func TestPriorityForQueue(t *testing.T) {
	tests := []struct {
		...
		{queue: queueCritical, want: jobs.PriorityHigh},
		{queue: queueDefault, want: jobs.PriorityNormal},
		{queue: queueLow, want: jobs.PriorityLow},
		{queue: "some-unknown-queue", want: jobs.PriorityNormal},
	}
	for _, tt := range tests {
...
func TestBuildTaskHeaders_And_HeaderCreatedAtTime_RoundTrip(t *testing.T) {
	...
	task := jobs.Task{Type: "t", TenantID: pkgcore.TenantID("tenant-a"), IdempotencyKey: "op-1"}
...
func TestEncodeDecodeResultEnvelope_RoundTrip(t *testing.T) {
	want := resultEnvelope{StartedAt: 12345, ProgressPct: 42, ProgressMsg: "halfway", Data: []byte("done")}
...
func TestDecodeResultEnvelope_EmptyOrMalformed_ReturnsZeroValue(t *testing.T) {
	...
	for name, data := range tests {
		t.Run(name, func(t *testing.T) {
			if got := decodeResultEnvelope(data); !reflect.DeepEqual(got, resultEnvelope{}) {
...
func TestAttemptsFromTaskInfo(t *testing.T) {
	tests := []struct {
		name    string
		state   asynqlib.TaskState
		retried int
		want    int
	}{
		{name: "pending, never dequeued", state: asynqlib.TaskStatePending, retried: 0, want: 0},
		{name: "scheduled, never dequeued", state: asynqlib.TaskStateScheduled, retried: 0, want: 0},
		{name: "aggregating, never dequeued", state: asynqlib.TaskStateAggregating, retried: 0, want: 0},
		{name: "active, first attempt", state: asynqlib.TaskStateActive, retried: 0, want: 1},
		{name: "active, third attempt (two prior failures)", state: asynqlib.TaskStateActive, retried: 2, want: 3},
		{name: "retry, one failure banked", state: asynqlib.TaskStateRetry, retried: 1, want: 1},
		{name: "retry, three failures banked", state: asynqlib.TaskStateRetry, retried: 3, want: 3},
		{name: "completed, succeeded on first attempt", state: asynqlib.TaskStateCompleted, retried: 0, want: 1},
		{name: "completed, succeeded after two prior failures", state: asynqlib.TaskStateCompleted, retried: 2, want: 3},
		{name: "archived, MaxRetry=3 exhausted (total attempts = MaxRetry+1)", state: asynqlib.TaskStateArchived, retried: 3, want: 4},
		{name: "archived, MaxRetry=0 (single attempt dead-letters immediately)", state: asynqlib.TaskStateArchived, retried: 0, want: 1},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			info := &asynqlib.TaskInfo{State: tt.state, Retried: tt.retried}
...
func TestStatusFromTaskState(t *testing.T) {
	tests := []struct {
		...
		{name: "pending", state: asynqlib.TaskStatePending, want: jobs.StatusPending},
		{name: "scheduled", state: asynqlib.TaskStateScheduled, want: jobs.StatusPending},
		{name: "aggregating", state: asynqlib.TaskStateAggregating, want: jobs.StatusPending},
		{name: "active", state: asynqlib.TaskStateActive, want: jobs.StatusRunning},
		{name: "retry", state: asynqlib.TaskStateRetry, want: jobs.StatusRetrying},
		{name: "completed", state: asynqlib.TaskStateCompleted, want: jobs.StatusSucceeded},
		{name: "archived", state: asynqlib.TaskStateArchived, want: jobs.StatusDeadLetter},
		{name: "cancelled wins over pending", state: asynqlib.TaskStatePending, cancelled: true, want: jobs.StatusCancelled},
		{name: "cancelled wins over active", state: asynqlib.TaskStateActive, cancelled: true, want: jobs.StatusCancelled},
		{name: "cancelled wins over retry", state: asynqlib.TaskStateRetry, cancelled: true, want: jobs.StatusCancelled},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestJobFromTaskInfo_Succeeded(t *testing.T) {
	completedAt := time.Date(2026, 1, 2, 3, 0, 0, 0, time.UTC)
	createdAt := completedAt.Add(-time.Minute)
	env, err := encodeResultEnvelope(resultEnvelope{ProgressPct: 100, ProgressMsg: "done", Data: []byte("result-bytes")})
	if err != nil {
		t.Fatalf("encodeResultEnvelope() error = %v", err)
	}
	info := &asynqlib.TaskInfo{
		...
		State:       asynqlib.TaskStateCompleted,
		MaxRetry:    3,
		Retried:     1,
	...
	if job.Status != jobs.StatusSucceeded {
		t.Errorf("Status = %v, want %v", job.Status, jobs.StatusSucceeded)
	}
	...
	if job.Priority != jobs.PriorityHigh {
		t.Errorf("Priority = %v, want %v (queue=%q)", job.Priority, jobs.PriorityHigh, info.Queue)
	}
	if job.Attempts != 2 {
...
func TestJobFromTaskInfo_DeadLetter(t *testing.T) {
	...
	info := &asynqlib.TaskInfo{
		...
		State:        asynqlib.TaskStateArchived,
		MaxRetry:     1,
		Retried:      1,
	...
	if job.Status != jobs.StatusDeadLetter {
		t.Errorf("Status = %v, want %v", job.Status, jobs.StatusDeadLetter)
	}
	if job.Attempts != 2 {
...
func TestJobFromTaskInfo_CancelledOverridesUnderlyingState(t *testing.T) {
	...
	info := &asynqlib.TaskInfo{
		...
		Type:    "flood",
		Headers: map[string]string{headerTenantID: "tenant-a"},
		State:   asynqlib.TaskStateRetry, // still "alive" from asynq's own perspective.
		LastErr: "a prior attempt's failure",
	}

	job := jobFromTaskInfo(info, &cancelledAt)

	if job.Status != jobs.StatusCancelled {
		t.Errorf("Status = %v, want %v even though the underlying asynq state is %v", job.Status, jobs.StatusCancelled, info.State)
	}
	...
	if job.Error != "a prior attempt's failure" {
		t.Errorf("Error = %q, want the prior attempt's message preserved (jobs.Job.Error's own doc comment: Cancel does not clear it)", job.Error)
	}
}
