...
//
// Like DemoQueue, AsynqQueue is returned as its own concrete exported type,
...
// comment reserves exactly this freedom for "a Redis/asynq-backed
// production implementation"). Code that only needs the portable surface
// should still depend on Queue, not this type; see the compile-time
...
const (
	...
	// remains visible to Get() after it completes. Unlike DemoQueue, whose
	...
	// evicted. Unlike DemoQueue's SQLite row, this is not forever -- see
...
// DemoQueue's own Option -- a distinct type (not a reuse of DemoQueue's
...
// codebase (see observability.Option, DemoQueue's Option above).
...
// combined, the direct analog of DemoQueue's WithWorkerCount. Zero or
...
// tenant may be running at once, the direct analog of DemoQueue's
...
// call that does not use WithTimeout, the direct analog of DemoQueue's
...
// letting asynq's own exponential-backoff formula stand in for DemoQueue's
// hand-rolled backoffDelay rather than reimplementing one; WithBackoff
// (DemoQueue's equivalent knob) configures a formula, not a pluggable
// function, only because DemoQueue rolls its own -- this is the same
...
// Redis polling load for faster pickup -- the same trade-off DemoQueue's
...
// and DemoQueue's own tests lower WithPollInterval/WithBackoff.
...
// interface). Like NewDemoQueue, NewAsynqQueue performs no I/O of its own:
...
// h.Type(). Identical contract to DemoQueue.RegisterHandler, including
...
// DemoQueue.Start -- a registration failure is logged and does not prevent
...
// second call is a no-op, matching DemoQueue.Start's own contract, though
...
// comment for the one behavioral nuance from DemoQueue.Close worth calling
...
// for that to finish, exactly like DemoQueue.Close, but does not itself
...
// call more than once, or without a prior Start, exactly like DemoQueue.
...
func (q *AsynqQueue) Enqueue(ctx context.Context, task Task, opts ...EnqueueOption) (JobID, error) {
	...
	// See DemoQueue.Enqueue's identical fix for the full rationale: derive
	...
	// absent) for both AsynqQueue.Enqueue and DemoQueue.Enqueue alike.
...
// mirroring DemoQueue's own markCancelled + completeSucceeded/
...
// DemoQueue's own documented "does not preempt" limitation (Queue.Cancel's
...
// DemoQueue.DeadLetterJobs, mapping onto asynq's own archived-task
...
// exactly like DemoQueue's version. Fully drains each queue's archive
...
// pagination" shape DemoQueue.DeadLetterJobs documents as a known
// limitation.
...
// DemoQueue.registerQueueDepthGauge does (docs/internal/09-observability.md's
// must-instrument table), reusing this package's shared instrumentationName
// constant (demo_queue.go). The label set differs from DemoQueue's, and
...
// priority tiers), not per job TYPE the way DemoQueue's own SQL GROUP BY
...
// DemoQueue's identical assertion.
