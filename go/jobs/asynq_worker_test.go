...
// newTestAsynqQueue returns a bare *AsynqQueue with only the fields the
// function under test reads populated -- no Redis, no asynq.Client/Server/
// Inspector construction. This is deliberately NOT NewAsynqQueue: every
// function tested from this file (tryReserveTenantSlot/releaseTenantSlot,
...
// asynq.RedisConnOpt) would only add noise. Anything that DOES need them --
// Enqueue/Get/Cancel, and processTask's own ResultWriter.Write calls past
// its early-exit branches -- is exercised in integration_test/ against a
// real Redis instead, per this package's own testing convention.
func newTestAsynqQueue(t *testing.T) *AsynqQueue {
	t.Helper()
	return &AsynqQueue{
...
func TestAsynqQueue_TryReserveTenantSlot_And_Release(t *testing.T) {
...
func TestAsynqQueue_TenantSlotReservation_ConcurrentAccessIsRaceFree(t *testing.T) {
	q := newTestAsynqQueue(t)
...
func TestIsFailure(t *testing.T) {
	tests := []struct {
		...
		{name: "wrapped tenant at capacity", err: errAsynqWrap(errTenantAtCapacity), want: false},
	}
...
func errAsynqWrap(err error) error {
...
func TestAsynqQueue_RetryDelay(t *testing.T) {
	q := newTestAsynqQueue(t)
	q.throttleRetryDelay = 100 * time.Millisecond
	businessCalled := false
	q.businessRetryDelayFunc = func(n int, e error, task *asynq.Task) time.Duration {
		businessCalled = true
	...
	t.Run("wrapped throttle error is still recognized", func(t *testing.T) {
		businessCalled = false
		got := q.retryDelay(0, errAsynqWrap(errTenantAtCapacity), task)
		if businessCalled {
...
// recordingFailureHook is a Handler+FailureHook test double recording
// every OnFailure call it receives.
...
func (h *recordingFailureHook) Handle(context.Context, *Job, ProgressFn) (Result, error) {
	return Result{}, errors.New("not exercised by this test")
}

func (h *recordingFailureHook) OnFailure(_ context.Context, job *Job, _ error) {
	h.calls = append(h.calls, job)
}

func TestAsynqQueue_HandleErrorAttempt(t *testing.T) {
	t.Run("retries remain: FailureHook must not fire", func(t *testing.T) {
		...
		task := asynq.NewTaskWithHeaders("always-fails", []byte("payload"), map[string]string{headerTenantID: "tenant-a"})
	...
	t.Run("retries exhausted: FailureHook fires exactly once with a DeadLetter Job", func(t *testing.T) {
		...
		task := asynq.NewTaskWithHeaders("always-fails", []byte("payload"), map[string]string{
		...
		got := h.calls[0]
		if got.ID != JobID("job-1") || got.TenantID != pkgcore.TenantID("tenant-a") || got.IdempotencyKey != "op-1" {
			t.Errorf("OnFailure job = %+v, want ID=job-1 TenantID=tenant-a IdempotencyKey=op-1", got)
		}
		if got.Status != StatusDeadLetter {
			t.Errorf("OnFailure job.Status = %v, want %v", got.Status, StatusDeadLetter)
		}
		if got.Attempts != 4 {
	...
	t.Run("no registered handler: no panic, no hook call", func(t *testing.T) {
		q := newTestAsynqQueue(t)
		task := asynq.NewTaskWithHeaders("unregistered-type", nil, map[string]string{headerTenantID: "tenant-a"})
	...
	t.Run("handler without FailureHook: no panic, no hook call", func(t *testing.T) {
		q := newTestAsynqQueue(t)
		plain := NewHandlerFunc("plain", func(context.Context, *Job, ProgressFn) (Result, error) { return Result{}, nil })
		if err := q.RegisterHandler(plain); err != nil {
			t.Fatalf("RegisterHandler() error = %v", err)
		}
		task := asynq.NewTaskWithHeaders("plain", nil, map[string]string{headerTenantID: "tenant-a"})
		q.handleErrorAttempt(task, errors.New("boom"), 3, 3, "job-1") // must not panic.
	})
	...
	t.Run("tenant-at-capacity bounce is never treated as dead-letter-worthy", func(t *testing.T) {
		...
		task := asynq.NewTaskWithHeaders("always-fails", nil, map[string]string{headerTenantID: "tenant-a"})
...
func TestAsynqQueue_ProcessTask_HandlerNotRegistered(t *testing.T) {
	q := newTestAsynqQueue(t)
	task := asynq.NewTaskWithHeaders("no-such-type", nil, map[string]string{headerTenantID: "tenant-a"})

	err := q.processTaskUncancelled(context.Background(), task, "job-1", obs.FromContext(context.Background()))

	appErr, ok := apperr.As(err)
	if !ok || appErr.Code != ErrHandlerNotRegistered.Code {
		t.Errorf("processTask() error = %v, want ErrHandlerNotRegistered (code %q)", err, ErrHandlerNotRegistered.Code)
	}
}

func TestAsynqQueue_ProcessTask_MissingTenantHeader(t *testing.T) {
	q := newTestAsynqQueue(t)
	handleCalled := false
	h := NewHandlerFunc("t", func(context.Context, *Job, ProgressFn) (Result, error) {
		handleCalled = true
		return Result{}, nil
	})
	if err := q.RegisterHandler(h); err != nil {
	...
	task := asynq.NewTaskWithHeaders("t", nil, map[string]string{}) // no tenant_id at all.
...
func TestAsynqQueue_ProcessTask_TenantAtCapacity_BouncesWithoutCallingHandle(t *testing.T) {
	q := newTestAsynqQueue(t)
	...
	task := asynq.NewTaskWithHeaders("t", nil, map[string]string{headerTenantID: string(tenant)})
