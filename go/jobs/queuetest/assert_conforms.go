...
// of via two independently hand-maintained test files. The healthy-path
// suite is only half of that agreement: AssertConforms never makes an
// underlying state read fail, so failure-direction divergence between
// implementations -- asynq's Get reporting a Job's natural state when the
// cancellation-marker read fails, where StandaloneQueue fails closed -- is
// invisible to it. The package's second entry,
