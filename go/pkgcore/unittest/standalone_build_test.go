...
// pkgcore: it is a repo-shape guard, not a test of pkgcore's own symbols.
package unittest
...
// own directory: the check lives in go/pkgcore/unittest/, whose go.mod the
...
// directory, one level below the module root, so the file's own directory is
// not where the module's go.mod sits.
