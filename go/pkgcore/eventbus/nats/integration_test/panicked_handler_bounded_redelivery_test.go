...
// on the NATS-backed bus (P1 finding: eventbus/nats unbounded redelivery).
// The scenario every test drives: a message whose remote delivery invokes a
// handler that panics. On the pre-fix code deliverRemote left such a message
// unacknowledged -- never acked, never terminated -- and the consumer
// carried no MaxDeliver (the JetStream server default is unlimited), so
// JetStream redelivered the message after every AckWait and every
// redelivery re-ran the WHOLE fan-out: the panicking handler re-ran without
// bound and dragged every healthy sibling handler along with it, and with
// enough accumulated unacknowledged messages the consumer suspended
// entirely at MaxAckPending -- a permanent stall of the type on that
// replica (measured on the pre-fix code: with the broker's AckWait
// shortened to test scale the panicking handler and its healthy sibling
// re-ran every ~1s forever). The redis twin test asserts the property that
// matters across backends -- later entries still reach healthy handlers --
// and eventbus/postgres's own bounded-retry round (febc068) established the
// bounded shape this suite replicates for JetStream.
//
// Post-fix semantics, pinned by the tests below: a panicked delivery is
// never acked as delivered, but its redelivery is bounded. deliverRemote
// negatively acknowledges the message (redelivered at panicRedeliveryDelay
// intervals) and records the panicked handler VALUES; every redelivery
// re-invokes ONLY those values, never the message's whole fan-out, so
// healthy siblings run exactly once and later same-type messages keep
// reaching them exactly once. The consumer's own MaxDeliver
...
// PRE-fix leave-unacknowledged behaviour (which redelivers only when
// AckWait expires) shows itself in seconds instead of the server default's
// 30s, while the post-fix negative-acknowledgement pacing is independent of
// AckWait and unchanged by it.
...
// full redelivery cycles (a pre-fix redelivery lands on every shortened
// AckWait expiry, about a second apart) plus margin, long enough that a
// message still being redriven would have surfaced another change inside
// the window.
...
// On the pre-fix code no deadline suffices -- the redeliveries never stop.
const settleDeadline = 25 * time.Second

// shortAckWait is the consumer AckWait every test's redelivery scaffolding
// sets: the pre-fix leave-unacknowledged path redelivers on AckWait expiry,
// so shortening it is what makes the pre-fix unbounded redelivery visible
// at test scale (the post-fix path paces its redeliveries by negative
// acknowledgements and is unaffected by AckWait).
...
// On the pre-fix code no such window exists for a panicking message's
// counters: the unacknowledged message is redelivered on every (shortened)
// AckWait expiry, re-running the whole fan-out, so any counter its delivery
// touches changes again within every ~1s redelivery cycle and the deadline
// expires instead.
...
// pre-fix leave-unacknowledged redelivery cadence fits the test window (see
// the file's doc comment). The update restates the consumer's own current
// configuration -- the bus created it -- changing only AckWait, so a
// consumer the pre-fix bus created (carrying no MaxDeliver, the unlimited
// server default) stays unlimited and the pre-fix unbounded redelivery
// under test is real, not capped by the scaffolding.
...
// pins the property the redis twin test asserts for its own backend (see
// the file's doc comment): after one message whose delivery panics, LATER
// messages of the same type must still reach the healthy handler, each
// exactly once. On the pre-fix code the panicked message was redelivered
// without bound, and every redelivery re-ran the whole fan-out -- so the
// healthy handler saw the panicked message again on every cycle, and the
// exactly-once assertion below fails the moment the first redelivery lands.
func TestEventBus_PanickingRemoteHandler_LaterSameTypeMessagesStillReachHealthyHandlers(t *testing.T) {
	...
	// only, never the healthy sibling, so no count below may move. RED
	// (pre-fix): every ~1s redelivery re-ran the whole fan-out, so the
	// healthy handler's count for the panicked message kept growing.
...
// exactly 1. RED (pre-fix): every redelivery cycle re-ran the message's
// whole fan-out, so the sibling's count never settled -- it grew on every
// ~1s AckWait redelivery without bound.
...
// unbounded redelivery loop and never an accumulating pile of
// unacknowledged messages. RED (pre-fix): the consumer carried no
// MaxDeliver (the assertion below catches that first: the consumer config
// reports the unlimited default), and the attempt count never stopped
// growing on the pre-fix code.
func TestEventBus_PanickingRemoteHandler_BoundedRetryThenLoggedTerminal(t *testing.T) {
	...
	// RED (pre-fix): the consumer was created without one, and JetStream's
	...
	// count must stay put through two full idle redelivery windows. RED
	// (pre-fix): it never settles.
