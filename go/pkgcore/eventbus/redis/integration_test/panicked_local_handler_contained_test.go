...
// recover of its own) -- contradicting this package's own claim that local
// handlers "behave exactly as they do on the in-memory bus", whose
// per-handler recover (pkgcore's runMemoryBusHandler) contains panics,
// logs them, and lets the handler's siblings still run. Post-fix the
// fan-out runs each handler through runLocalHandler, which recovers the
// panic, logs it, drops it (never an error a caller could act on), lets the
// handler's siblings still run, and keeps the publish a success.
...
// containment contract against a real Redis server: a Publish whose first
...
func TestEventBus_PanickingLocalHandler_ContainedSiblingsRun(t *testing.T) {
	...
	const eventType = "eventbus_redis_test.local_panic_contained"
