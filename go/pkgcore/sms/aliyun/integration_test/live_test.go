...
//	ALIYUN_SMS_TEMPLATE_CODE        an approved template's code
...
//
//	ALIYUN_SMS_MESSAGE_ID           the message id the template is mapped
//	                                for (default
//	                                "authn.sms.verification_code")
//	ALIYUN_SMS_LOCALE               the locale it is mapped for (default
//	                                "zh-CN")
//	ALIYUN_SMS_TEMPLATE_VARS        the comma-separated variable names the
//	                                template declares (default "code,minutes");
//	                                each declared variable is sent with a
//	                                plausible value -- "code" gets 123456,
//	                                "minutes" gets 5, anything else gets the
//	                                variable's own name
//
...
const (
	...
	envMessageID       = "ALIYUN_SMS_MESSAGE_ID"
	envLocale          = "ALIYUN_SMS_LOCALE"
	envTemplateVars    = "ALIYUN_SMS_TEMPLATE_VARS"
	envTo              = "ALIYUN_SMS_TO"
)
...
// flows deliver, whose template declares code and minutes.
const (
...
func configFromEnv(t *testing.T) aliyun.Config {
	...
	cfg := aliyun.Config{
		...
		Templates: map[string]aliyun.Template{
			locale + "/" + messageID: {
				Code:   os.Getenv(envTemplateCode),
...
// (locale, message-id) key, with a plausible value for each variable the
...
func liveMessage(cfg aliyun.Config, to string) pkgcore.SMS {
...
// the property this leg exists to prove. A signature, encoding, placement or
// template-mapping defect fails loudly with the gateway's own error text,
// and a successful send is the acceptance.
func TestSender_LiveAliyunGateway_SendsOneMessage(t *testing.T) {
	cfg := configFromEnv(t)

	sender, err := aliyun.NewSender(cfg)
	if err != nil {
		t.Fatalf("NewSender: %v (a config error here means the %s/%s/%s/%s values do not satisfy the documented Config shape)", err, envAccessKeyID, envSignName, envTemplateCode, envTemplateVars)
	}

	err = sender.Send(context.Background(), liveMessage(cfg, os.Getenv(envTo)))
	if err != nil {
		t.Fatalf("Send against the real Aliyun SMS gateway: %v (if this is an isv.*/SignatureDoesNotMatch business error, the AccessKey in %s must have SendSms permission, %s must be an approved signature, %s an approved template code for the %s/%s message, and %s must list exactly the variables that template declares)", err, envAccessKeyID, envSignName, envTemplateCode, envLocale, envMessageID, envTemplateVars)
	}
}
