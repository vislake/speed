...
- Do not let one locale drift from another. Every language a module ships must carry the same id set; `Builder.AddModule` fails with `ErrParityMismatch`, and `tools/check_i18n_keys.py` checks the same zh-CN/en-US parity over the raw files when it is run (it is wired into no workflow yet -- its home is the gated docs-check pipeline, docs/internal/18-cicd.md -- so it runs directly).
...
| `i18n.ErrParityMismatch` | The id sets of one module's locale files differing | The error names the languages and the missing ids; `tools/check_i18n_keys.py` checks the same zh-CN/en-US parity over the raw files when it is run (its workflow home is the gated docs-check pipeline, docs/internal/18-cicd.md) |
