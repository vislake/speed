...
Built-in implementation names: `eventbus.memory`, `eventbus.redis`, `kv.memory`, `kv.redis`, `mailer.console`, `mailer.smtp`, `objectstore.local`, `objectstore.s3` -- each a thin `Config`-consuming adapter over an unchanged typed constructor (`NewMemoryEventBus`, ...). `eventbus.memory`, `kv.memory`, `mailer.console`, `mailer.smtp` and `objectstore.local` register here, in `builtin_implementations.go`; `eventbus.redis`, `kv.redis` and `objectstore.s3` register themselves from their own subpackages (`eventbus/redis`, `kv/redis`, `objectstore/s3` -- see this file's file-location table above), so they are visible on `EventBusRegistry`/`KVStoreRegistry`/`ObjectStoreRegistry` only once a host has imported the matching subpackage. The two Redis-backed adapters fall back to `"localhost:6379"` when `cfg["addr"]` is unset; the SMTP and S3 adapters have no safe default credentials and fail with `ErrMissingSeamConfig` instead of calling their panicking constructor with an empty field.
...
Every in-process or SMTP built-in implementation runs its matching suite in this package's own unit tier: `eventbus_conformance_test.go`, `kv_conformance_test.go`, `mailer_conformance_test.go` and `objectstore_conformance_test.go` (package `pkgcore_test` -- an external test package, because each imports the seam's `*test` package, which itself imports `pkgcore`, and an internal `package pkgcore` test file importing that back would be an import cycle) run `NewMemoryEventBus`/`NewMemoryKVStore`/`NewConsoleMailer`+`NewSMTPMailer`/`NewLocalObjectStore` through `AssertConforms`. The SMTP leg needs no Docker: it scripts an in-process fake relay (`internal/testutil/fake_smtp_server.go`), so it lives in the plain unit tier alongside the other three memory/console/local legs. The three Redis- and S3-backed implementations run the identical call against a real Redis/RustFS container from their own subpackage's own `integration_test/` (`-tags=integration`, Docker required) -- `eventbus/redis/integration_test/`, `kv/redis/integration_test/` and `objectstore/s3/integration_test/` -- rather than a shared one here, per this repository's testing standard that a package's integration tests live in its own `integration_test/` directory. `kv/postgres/integration_test/` runs the same call against a real PostgreSQL container, alongside the two adversarial concurrency proofs `TestKVStore_ConcurrentIncrementsLoseNoUpdates` and `TestKVStore_ConcurrentCompareAndSwapSetIfAbsent_ExactlyOneWinner` that `AssertConforms` alone (effectively single-threaded) cannot exercise. A host registering a new implementation on `EventBusRegistry`/`KVStoreRegistry`/`MailerRegistry`/`ObjectStoreRegistry`, or injecting one directly through `WithEventBus`/`WithKVStore`/`WithMailer`/`WithObjectStore`, is expected to run the matching `AssertConforms` against it the same way, per this file's own Rules section.
