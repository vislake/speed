...
// smtpMailerFromConfig adapts Config onto NewSMTPMailer: it reads the flat
// string settings, defaulting an unset port to the submission port, and
// hands the typed fields to newSMTPMailerFromFields (mailer_smtp.go), the
// validation-and-construction path the "mailer.smtp" component shares.
// Host has no safe default -- there is no such thing as a generic SMTP relay
// -- so a Config missing it is rejected with ErrMissingSeamConfig before
// NewSMTPMailer is even called, rather than letting that constructor's own
// panic (its unrecoverable-wiring-error convention for a caller that built
// an SMTPConfig by hand) surface through a SeamRegistry.Build call that is
// documented to return an error, never to panic. A numeric port outside
// 1..65535 is refused for the same reason -- the constructor would panic
// over it -- and a "reply_to" carrying a line break, because the
// constructor's own line-break panic would otherwise be reachable through
// the error-returning seam path.
func smtpMailerFromConfig(cfg Config) (Mailer, error) {
	...
	return newSMTPMailerFromFields(cfg["host"], port, cfg["username"], cfg["password"], cfg["reply_to"], tlsMode, cfg["insecure_skip_verify"] == "true")
}
