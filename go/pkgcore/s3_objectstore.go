...
// S3Config names one bucket on an S3-compatible object service and the
...
// s3ObjectStore is the distributed deployment mode's ObjectStore: an
...
// missing key is then reported by GetObject itself as ErrObjectNotFound, the
// same timing the local store's open gives, not on some later read.
type s3ObjectStore struct {
	client *minio.Client
	bucket string
}

// NewS3ObjectStore returns the distributed deployment mode's ObjectStore,
...
// mode the SMTP mailer uses for an unusable configuration. The caller never
// imports minio-go: the store builds its own client and the minio types
// never cross this seam.
func NewS3ObjectStore(cfg S3Config) ObjectStore {
	if cfg.Endpoint == "" {
		panic("pkgcore: NewS3ObjectStore requires a non-empty S3Config.Endpoint")
	}
	if cfg.Bucket == "" {
		panic("pkgcore: NewS3ObjectStore requires a non-empty S3Config.Bucket")
	}
	if cfg.AccessKey == "" {
		panic("pkgcore: NewS3ObjectStore requires a non-empty S3Config.AccessKey")
	}
	if cfg.SecretKey == "" {
		panic("pkgcore: NewS3ObjectStore requires a non-empty S3Config.SecretKey")
	}
	...
	if err != nil {
		panic(fmt.Sprintf("pkgcore: NewS3ObjectStore: %v", err))
	}
	return &s3ObjectStore{client: client, bucket: cfg.Bucket}
}

// PutObject implements ObjectStore.PutObject by streaming r to the service.
// The size of r is unknown, so minio-go uploads it as a multipart upload
// whose parts stream from r as they are read; memory use is bounded by the
// part buffer, never by the object's size, and the context is honoured
// throughout, so a cancelled upload stops and is aborted server-side.
func (s *s3ObjectStore) PutObject(ctx context.Context, key string, r io.Reader) error {
	...
	if _, err := s.client.PutObject(ctx, s.bucket, key, r, -1, minio.PutObjectOptions{}); err != nil {
		return fmt.Errorf("pkgcore: s3 object store put: %w", err)
	}
	return nil
}

// GetObject implements ObjectStore.GetObject. minio-go's GetObject defers
// the request until the object's first read or Stat call, so the store calls
// Stat before returning: the request goes out now, a missing key comes back
// now as ErrObjectNotFound, and the caller receives a reader that is already
// streaming the object's bytes, whose own reads fail with the context's
// error once that context is done. The returned reader is minio-go's own,
// and the caller closes it exactly as the interface requires.
func (s *s3ObjectStore) GetObject(ctx context.Context, key string) (io.ReadCloser, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("pkgcore: s3 object store get: %w", err)
	}
...
// DeleteObject implements ObjectStore.DeleteObject. S3 DELETE is idempotent:
// removing a key that has no object is a 204 success on every compatible
// service, and a stray NoSuchKey (the odd gateway) is mapped onto the same
// success, keeping the interface's promise that deleting a missing key never
// fails.
func (s *s3ObjectStore) DeleteObject(ctx context.Context, key string) error {
	...
	if err := s.client.RemoveObject(ctx, s.bucket, key, minio.RemoveObjectOptions{}); err != nil {
		...
		return fmt.Errorf("pkgcore: s3 object store delete: %w", err)
	}
	return nil
}
