/**
 * window.matchMedia mock for tests that exercise the AppShell frame's
 ...
 * before rendering to pick the desktop or mobile branch
 * deterministically; the consuming package's setup.ts installs a
 * desktop-true default before each test, so suites that do not care
 * about the split need not call it at all.
 *
