 ...
 * for the harness in session-harness.ts.
 *
 ...
 * consuming packages' suites only ever script legitimate keys, so they
 * pin the gate's own-property branch while nothing drives its other
 * branch -- a key that exists only on the script's prototype chain must
 * take the missing-handler path, never dispatch onto an inherited
 * callable. This file drives exactly that branch: the request's method
 * is 'constructor', the best-known prototype-chain name, and the script
 * is built over a prototype that carries a callable under the precise
 * key the request derives. On an ungated read, the bare script[key] read
 * would have resolved that callable through the prototype and dispatched
 * it, so the rejection asserted here is the gate's own observable
 * behaviour.
 */
