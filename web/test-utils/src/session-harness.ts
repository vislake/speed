/**
 * session-harness.ts -- the shared scripting harness for tests that
 * drive an @speed/auth-core session. A fake request function bound
 * through the @speed/api-sdk runtime seam (bindRequestFn, the same seam
 * a host's real client uses) drives a real session over a fresh memory
 * store; tests assert observable state only: the store's token,
 * getSnapshot, subscriber notifications, the request script's bodies and
 * the raw ApiErrors a failed operation rejects.
 *
 * The harness covers the whole auth operation surface -- sign-in
 * (password, SMS code request and code, social authorize and callback),
 * registration, logout, token refresh, tenant switch and step-up -- so a
 * package's suite scripts only the operations its surface drives. The
 * endpoint path constants are literal request keys, pinned to 'google'
 * for the provider-embedding social endpoints: a test that needs another
 * provider writes its own literal key (the session threads whatever
 * provider string it is given through verbatim).
 */
...
import type { AuthSession, AuthSnapshot } from '@speed/auth-core'
