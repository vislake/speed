...
The layout's last entry is different by design: `api-sdk/` ships
**generated** source. `src/index.ts` is orval output from the notes
fragment — stamped with a DO-NOT-EDIT header carrying the pinned orval
version, regenerated by `task api:gen` from `web/` (pinned `pnpm dlx
orval@8.17.0` plus `scripts/orval-nodenext-fixup.mjs`; orval itself is
never a workspace dependency, so it never enters the lockfile) and never
hand-edited. Its only hand-written source is the seam `src/runtime.ts`,
kept out of orval's way by an exports-map `./runtime` subpath; the
package's AGENTS.md documents the seam and the fixup's why.
