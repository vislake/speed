/**
 * axe-core assertions for ui-kit component tests.
 *
 ...
 * a browser run would be (ConfirmDialog's open-dialog scan relies on
 * that shape); a closed but keepMounted drawer (visibility: hidden) is
...
async function runAxeScan(options: axe.RunOptions = {}): Promise<axe.AxeResults> {
  ...
  document.title = document.title || 'ui-kit test'
