/**
 * axe-core assertions for billing-ui component tests.
 *
 ...
 * elementsFromPoint with an empty hit stack -- the truthful answer for
 * a document jsdom lays out not at all -- and it detects a really open
 * overlay itself (a visible `[aria-modal="true"]` paper or any other
 * visible MUI Modal-based overlay root, see hasOpenOverlay) to
 * reproduce the browser's passForModal exemption, because the
 * modal-open part of axe's own probe cannot run under jsdom. The
 * consequence component suites must live with: a scan whose document
 * contains no h1 FAILS with a page-has-heading-one violation. A test
 * that renders a heading-bearing component in isolation must supply the
 * h1 ancestor context (and, where the component's heading level depends
 * on the page, the level that continues the order); where a component
 * legitimately renders without one the suite must assert that heading
 * situation explicitly in the render, never by disabling the rule
 * silently. A scan taken while a modal is really open is exempt from
 * the page-context rules exactly as a browser run would be; a closed
 * but keepMounted drawer (visibility: hidden) is NOT a modal and still
 * faces the real page rules.
 *
 * The widget-tier carve-out: `region` and the page-level main-landmark
 * rule (landmark-one-main) stay disabled below -- the same rationale as
 * region's, which is landmark containment, not widget semantics:
 * "landmark containment is a page-level concern; the units under test
 * are components, not full app pages". `page-has-heading-one` is
 * deliberately NOT part of that carve-out, because these components DO
 * render real heading elements whose levels only make sense relative to
 * a page heading -- the scan must know that context to be honest.
 *
...
async function runAxeScan(options: axe.RunOptions = {}): Promise<axe.AxeResults> {
  ...
  document.title = document.title || 'billing-ui test'
