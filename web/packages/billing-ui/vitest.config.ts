/**
 * billing-ui test configuration: jsdom DOM environment with the shared
 * jest-dom matcher setup, plus the workspace-sibling source aliases
 * that mirror tsconfig.json's paths.
 *
 * The aliases point @speed/* specifiers at the siblings' src entry
 * files so tests run against live sources (a sibling's dist/ is never
 * committed and not guaranteed to exist when tests run). src/ resolves
 * @speed/api-sdk and @speed/i18n; test-utils/ renders under the ui-kit
 * theme providers -- whose own sources import @speed/tokens and
 * @speed/i18n/mui-locale, aliased below for the same reason -- and
 * binds a real client through the api-client and api-sdk seam.
 * Aliases match by prefix, so a subpath entry must come before the
 * entry that is its prefix: "@speed/i18n/mui-locale" before
 * "@speed/i18n", and "@speed/api-sdk/runtime" before "@speed/api-sdk".
 */
