 ...
 * and every such check reports "incomplete" -- indeterminate,
 * never failing, no matter what the component rendered:
 * page-has-heading-one could never make a test fail, and the isolated
 * widget renders in this package's suites are exactly that shape. The
 * helper restores determinacy two ways: it
 ...
 *
 * Unlike the widget-tier helpers, `region` stays ENABLED here by default
 * -- ProductShell's authenticated branch is page-level chrome with repeated landmarks (banner, nav, main) -- and the page-level main-landmark rule
 * (landmark-one-main) stays ENABLED for the same reason: the frame
 * renders a real `main`, so the scans can and must answer it.
 * `page-has-heading-one` is live too: a page-shaped scan's document IS a
 * page, the chrome renders no page heading of its own, so the test must
 * supply the host content (inside `main`) that does.
 *
...
export async function expectNoAxeViolations(
  ...
  // region and landmark-one-main stay enabled here by default (see the
  // header): the page-shaped scans can and must answer them. Pass them
  // in disabledRules for the rare deliberately incomplete fragment.
  const disabled = new Set<string>(['color-contrast', ...disabledRules])
  if (hasOpenOverlay()) {
