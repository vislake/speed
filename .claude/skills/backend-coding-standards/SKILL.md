...
- **What a unit test is, is a tier — not a file mapping.** A unit test is a same-package, no-external-dependency test that a plain unit run executes (the standalone deployment mode's in-memory implementations act as test doubles). Unit tests alone sit next to the code they test, one file per target — `registry.go` is tested by `registry_test.go`, `kv.go` by `kv_test.go`, in the standard Go idiom. That includes in-package behaviour suites that span sources rather than mapping 1:1 to one file: they stay in the package, named for the behaviour they verify, never a generic word (`extra_test.go`, `misc_test.go`).
- **Every non-unit test class lives in its own dedicated directory; nothing non-unit is ever mixed into a source directory.** If no existing dedicated directory fits a non-unit test, give it a new purpose-named directory — never a seat beside the code:
  ...
  - cross-implementation contract suites live in the seam's own support package — the `eventbustest`/`queuetest` model, where the shared `AssertConforms`/`AssertFailsClosed` checks are importable, black-box, from that package and every implementation's driver runs them against the implementation;
  - migration suites stay beside the migration set they exercise.
- **Two classes cannot move by Go's own package rules and are explicit in-package exceptions under naming discipline:** godoc `Example*` functions live in `example_test.go` — Go's own idiomatic name for godoc-rendered runnable examples, do not rename it — and white-box benchmarks that must reach unexported symbols live in `<target>_bench_test.go` beside their target. (An external-test-package case — `package x_test` — that must import a support package which imports `x` back can likewise not share a file with internal `package x` cases; that mechanical split still names its file after what it verifies.)
- Within the co-located unit tier, keep **one test file per source file, full stop**: every test for `registry.go`, however many scenarios or sub-behaviours it covers (concurrency, edge cases, a specific serialization format, whatever), goes in the one `registry_test.go`. Do not split one target's tests by theme — a target with 30 test functions covering ten different scenarios is still one file, not ten. Splitting by semantic grouping is a habit to actively resist, not a style choice to make freely.
  - If a unit-tier suite genuinely spans targets (a cross-cutting behaviour over several source files), it may live as one behaviour-named file (`subscribe_lifecycle_test.go`) beside the sources it exercises — never as a theme-split of any single target's file.
