...
- **Every non-unit test class lives in its own dedicated directory; nothing non-unit is ever mixed into a source directory.** If no existing dedicated directory fits a non-unit test, give it a new purpose-named directory — never a seat beside the code:
  ...
  - cross-implementation contract suites live in the module's own support package — the `eventbustest`/`queuetest` model, where the shared `AssertConforms`/`AssertFailsClosed` checks are importable, black-box, from that package and every implementation's driver runs them against the implementation. A subpackage implementation's driver lives in its own subpackage's tier; a driver of the module root's own built-ins is a no-target unit test and lives in the module's `unittest/` directory (`go/pkgcore/unittest/eventbus_conformance_test.go` is the template);
  - migration suites stay beside the migration set they exercise.
- **Go's own package rules force a small set of in-package exceptions; every one is explicit, named and recorded** (the per-file inventory with reasons lives in `docs/internal/25`): godoc `Example*` functions live in `example_test.go` — Go's own idiomatic name for godoc-rendered runnable examples, do not rename it; white-box benchmarks that must reach unexported symbols live in `<target>_bench_test.go` beside their target; an external-test-package case (`package x_test`) that must import a support package which imports `x` back can likewise not share a file with internal `package x` cases — that mechanical split still names its file after what it verifies; and a white-box unit suite the no-target rule cannot move stays in the package under a behaviour name (`go/observability/factory_vars_test.go` is the template: it swaps `init.go`'s unexported exporter-factory package vars, which no external test package can reach).
- Within the co-located unit tier, keep **one test file per source file, full stop**: every test for `registry.go`, however many scenarios or sub-behaviours it covers (concurrency, edge cases, a specific serialization format, whatever), goes in the one `registry_test.go`. Do not split one target's tests by theme — a target with 30 test functions covering ten different scenarios is still one file, not ten. Splitting by semantic grouping is a habit to actively resist, not a style choice to make freely.
  - If a unit-tier suite spans targets with a clear dominant source, merge it into that source's own test file; if it has no dominant source it is a no-target suite and lives in the module's `unittest/` directory (above) — never as a stray behaviour-named file in the source package, never as a theme-split of any single target's file.
  - If the one file has grown so large it is genuinely difficult to navigate (a high bar — well past a thousand lines, not a soft 600–800 line trigger; when in doubt, don't split), a mechanical split still keeps the target's name as a prefix (`registry_bootstrap_test.go`, not bare `bootstrap_test.go`) — never a generic word.
