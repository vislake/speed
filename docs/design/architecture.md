...
启用集必为可用集的子集，由各模块在 `Prepare` 阶段依据自己的配置表态、再经排他冲突消解得出，并始终含配置模块。没有一份集中的模块清单，模块自己最清楚它需要的配置是否齐备。
...
启动诊断是本框架唯一的可观测面。未进入启用集的模块必须在诊断中列出，并给出它表态禁用或消解落选的原因；这是"启动起来了却缺一部分功能且无人察觉"这一失败的唯一防线（[ADR-module-enablement-2026-09-13](../adr/adr-module-enablement-2026-09-13.md)）。
