...
标注为敏感的输入项，`config` 不把它写进自己的诊断，帮助输出中也不回显其默认值。**`config` 的保证到此为止**：模块 `Decode` 之后拿到的是普通结构体，那个值此后去了哪里，由拿到它的模块负责。跨模块的脱敏由日志模块承担，它按类别声明覆盖范围而不声称完整（[log](modules/design-log.md)）。
