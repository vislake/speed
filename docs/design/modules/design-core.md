...
可赋值性在另外两处仍然成立，与此不矛盾：模块的产物必须可赋值给它所声明的功能接口，这是交付断言，也是取用所得的值断言成该接口必定成功的依据（[ADR-capability-token-2026-09-13](../../adr/adr-capability-token-2026-09-13.md)）；资源查询同样按可赋值性，见资源查询一节。两者说的是值与类型的关系，不是一个标识与另一个标识的关系。
