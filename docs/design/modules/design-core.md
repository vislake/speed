...
- **查询**：按名称取描述符、按功能取实例、按类型取资源、遍历全部已注册模块。
...
- **不提供运行期参数。** 运行期可变的参数属于设置，由独立模块承担（[ADR-config-scope-2026-09-13](../../adr/adr-config-scope-2026-09-13.md)）。
...
**包级函数只有构造函数与泛型封装两类。** 注册表的构造函数无法是它自己的方法；泛型封装则是语言的限制所致，Go 不允许方法带类型参数，凡要靠类型参数换取类型安全的封装都只能写成自由函数。除这两类之外不新增包级函数，泛型封装也不得携带方法之外的任何语义。
...
`Run` 推进全部阶段，阻塞运行，`ctx` 取消后逆序关闭并返回。生命周期是一个不可分割的过程而非一对可分别调用的操作：进去就一定会出来，资源释放不依赖宿主记得调用第二个方法。宿主的 `main` 因此负责 import 所需模块、声明自己的宿主身份（见 [config](design-config.md)）、调用 `Run`、以及决定 `ctx` 何时取消。
...
它已在 `Prepare` 中构造，`New` 阶段跳过它；其余阶段照常执行，关闭时排在最后，因为任何模块都可能在关闭途中读配置。注册表中没有名为 `config` 的模块时启动失败，错误指明缺少的 import。
...
禁用必须给出原因，它随启动诊断输出。"启动起来了却缺一部分功能且无人察觉"是配置驱动最危险的失败，一行"模块 X 未启用，因为未配置连接地址"就把它挡在了排查之前。

启用集是表态非禁用、且未被传播禁用的模块，加上配置模块。不交付任何功能的模块不参与消解，只看自己的表态。
...
构造完成后，注册表断言该模块声明的每个 `Provides` 功能都已交付。声明与实际交付的偏离在此暴露，而不是留给后续某个取不到依赖的模块去发现。
...
注册期的错误在登记的那一刻 panic：模块重名，以及声明中出现不合法的 `Token`。绝大多数登记发生在 `init`，宿主自己的登记与测试的手工登记发生在 `main` 中，都在 `Run` 之前。panic 信息必须指明冲突双方的包路径：可用集由 import 决定，冲突的另一方可能是宿主的某个依赖 import 进来的，不在宿主自己的代码里，只报告模块名不足以定位。
...
`ErrMissingProvider` 的错误文本要区分成因：没有任何模块交付该功能，修法是 import 或启用一个提供者；有模块交付但它排在调用方之后构造，修法是把它声明进 `Requires` 或推迟到 `Init` 再取；提供者被消解禁用，修法见它被禁用的原因。判断所需的信息在描述符的交付声明里，注册表现成就有。
