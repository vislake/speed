...
返回的是最终结果而非模块自己的表态：被消解禁用的模块在此表现为 `StateDisabled`，原因是消解给出的那一条。消解尚未发生、以及消解失败时，一律返回 `false`，与查一个未登记的名字同形：它承诺的是最终结果，而最终结果此时并不存在，给出任何值都会被当成结论。尤其不能给出零值表态，那会把「没算出来」读成「表态为 `StateAuto`」。启动诊断固定写 `os.Stderr`，宿主无法重定向它，因为传入一个接收端只能经由 `Run` 的参数或包级设值，两者都被架构不变量排除。这个查询是宿主把同一批信息接进自己日志的合法途径：诊断保证信息不会无人察觉，查询让它可被程序取用。
...
禁用必须给出原因，它随启动诊断写入 `os.Stderr`，每个未启用的模块一行。**这一轮诊断在消解算出启用集之后写出**，因此消解本身失败时不会有它，此时点名冲突双方的是错误文本。
