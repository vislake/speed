...
1. 该端的路由表挂到路由引擎。同一个端上的模式冲突在此暴露，启动失败并点名冲突的两条模式与它们所在的监听端。是否冲突由路由引擎判定，本模块不自己比对。
...
**未落到任何一层的约束要在启动诊断中列出。** 一条 `After` 或 `Before` 指向的功能，若本次装配中没有任何一层用 `Provides` 声明代表它，该约束无声地消失——声明它的那一层以为自己有位置保证，实际没有。这不构成失败（功能缺席是合法配置），但它必须能被看见，否则与漏写 `Provides` 的后果无从分辨。
