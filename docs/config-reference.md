...
How it is built: the dynamic layer is enumerated through `config.Service.Describe` from the frozen schema of a composed host that registers every platform module declaring configuration items (authn, metering, compliance, sharing and pki -- see the generator's own header comment in `examples/reference-app/cmd/configrefgen/`); the bootstrap layer is the reference app's own environment surface, its inventory walked out of the app source and its per-variable facts curated in the same generator under a coverage gate. The two committed outputs (`docs/config-reference.md` and `docs/config-reference.json`) and the root `.env.example` are byte-identical across runs; regeneration is `go run ./cmd/configrefgen` from `examples/reference-app/`.
...
| `APP_ROOT_KEY` | hexkey | individual development defaults | yes | Single root secret from which all six key materials below are derived (HKDF-SHA256, one purpose string per key); an explicitly-set individual key always wins over its derivation. |
...
<!-- Generated by examples/reference-app/cmd/configrefgen (go run ./cmd/configrefgen from examples/reference-app/). Do not hand-edit. Reference stats: 35 bootstrap variable(s), 40 dynamic item(s)/flag(s) (8 flag(s), 5 sensitive) across 5 declaring module(s). Drift gate: docs-check.yml runs the generator with --check. -->
