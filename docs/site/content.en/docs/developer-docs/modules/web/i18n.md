...
the `en-US` default. The order is the design: explicit intent outranks
...
- An unknown browser language resolving to the `en-US` default is a
  negotiation default — a documented, overridable floor every chain
  terminates at. It is a different rule from the missing-key rule, which
  never falls back.
...
flowchart LR
    ...
    N --> D["default en-US<br/>(negotiation default, not a fallback)"]
