...
| `pki` | Signing-key and X.509 certificate lifecycle: the `Signer` seam, internal CA issuance, revocation, CRL generation and JWKS export. `authn`'s `KeySource` is its key-lifecycle layer's real, frozen-API consumer; the X.509/CA layer's consumer is the reference app's AI-output attestation layer (CA bootstrapping, per-tenant certificate issuance and signing, chain-verified gating of public shares), which drives the certificate-revoke and CRL-fetch HTTP operations. | [AGENTS.md](https://github.com/vislake/speed/blob/main/go/pki/AGENTS.md) |
