...
- **The structural check now validates the built output**:
  ...
  pinned Hugo binary must be installed first) and checks
...
- **Local preview**: `hugo server --minify --source docs/site` (or the
  ...
  pinned Hugo binary (`.mise.toml`'s `hugo` entry) and the
  theme submodule checked out (`git submodule update --init` if this
  clone predates the submodule).
- **The error-code index is now a site page**: the human-readable
  index `tools/gen_error_code_index.py` generates is committed as the
  site's own `content.en/docs/user-guide/error-codes.md` (served at
  `/docs/user-guide/error-codes/`, with a fixed Hugo front matter block
  the generator prepends); its machine-readable twin stays in the
  repository at `docs/error-codes.json`, read by the reference app's
  codes-alignment suite. A Chinese companion page explains the
  conventions and links the English table. `--check` and the coverage
...
- **Still deferred to M4** (docs/internal/13-documentation-standards.md):
  ...
  and gated by its own `--check`) and the ADRs surfaced
...
`tools/check_docs_site.py` runs before validating it, and what a
publisher runs before uploading the result.
...
script's own docstring for the full list of properties it checks. Run
it whenever this site, the checker itself or the pinned Hugo version
changes.

## Publishing

Publishing this site is a manual operation: build it (`hugo --minify
--gc` from this directory) and upload Hugo's output directory
(`docs/site/public`), not `docs/site/` itself.

## What the pages cover

- Home (`content.<lang>/_index.md`) — landing page and site map,
  routing the two main sections and the AI-agent entry point.
- `docs/_index.md` — the documentation hub linking User guides and
  Developer docs.
- `docs/user-guide/_index.md` — the User-guides hub: two reading paths
  (by domain for speed of entry, by module for completeness).
- `docs/user-guide/error-codes.md` — GENERATED by
  `tools/gen_error_code_index.py` (run from the repository root; the
  committed file includes the generator's fixed front matter block and
  an HTML-comment provenance footer). Do not hand-edit; the
  generator's `--check` mode fails when it drifts. The Chinese companion
  `content.zh-cn/docs/user-guide/error-codes.md` is hand-written and
