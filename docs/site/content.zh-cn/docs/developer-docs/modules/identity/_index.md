...
本组的定义性属性是它**不**依赖什么。`authn`、`rbac`、`org` 之间没有任何 Go import 边——两个方向都没有——这个"没有"是设计,不是打包的偶然结果。每条缺失的边各有各的理由,而本该由那条边承载的协作,全部经由宿主注入的接缝(seam)跨过边界:
...
- **`authn` 从不 import `org`。** 成员关系经由宿主注入的 `MembershipReader` 接缝询问,org 的名册是它的规范实现。接缝缺失意味着**拒绝,绝不放行**:令牌为之签发的那个租户,是多租户产品里被利用最多的横向越权入口,所以这个问题在每次登录时被问、在每次刷新时被再问,问的方式是失败即拒。
...
- **`authn` 以同样的方式够到 `pki`。** 它的 `KeySource` 接缝在 authn 内声明,由 `go/pki` 的 `Service` 结构化满足——两个方向都没有 import 边;宿主在组装时把它们接上(见 authn 设计页的令牌一节)。

这条模式在每条边上用的是同一种手法:接缝接口的签名只用标准库类型,由消费方声明、产出方实现、宿主接线——宿主是唯一同时出现两个包名的地方。参考应用里,演示身份层填的正是真实消费者会用真实模块去填的那些接缝——这正是接缝存在的意义。
...
flowchart LR
    ...
    O -.->|成员答案,经宿主 MembershipReader 接缝| A
    O -.->|节点路径,经宿主 SubtreeResolver 接缝| R
    P -.->|密钥,经 KeySource 接缝| A
