...
title: 工具组
weight: 6
description: "模块设计区最末的一组:saasctl——面向业务方的 CLI。它是唯一永不组装进运行服务的交付物,以及它为什么仍然随库发布。"
...
模块或 npm 包:它们携带 `Component` 描述符、注册进装配的 `ComponentRegistry`、
在装配里启动。本组文档化的是唯一一个都不做的交付物——
**saasctl**,面向业务方的 CLI。它不带组件,也不带任何
...
上,在启动之前与启动之外——从不进入运行中的服务。它的消费方是生
