...
- **根包不背导出器 SDK。** 两个导出器族各居子包,空白导入以取 `init` 副作用——`database/sql` 驱动模式,每类模块一个注册槽,因为本包每类恰好只有一套内置实现。只做日志的消费者永不继承 gRPC/protobuf 或 Prometheus 依赖树;depguard 规则只豁免所属子包,把任一 SDK import 回根包的改动会立刻挂 lint。
