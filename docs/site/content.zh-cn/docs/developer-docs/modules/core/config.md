...
- **引导解析不是本模块的事。** 如何够到基础设施——DSN、地址、部署模式、组合配置、引导密钥材料——在进程启动时一次性决定:键声明在每个组件 `ConfigSchema` 的 `derive` 字段上、由通用加载器 `pkgcore/config` 解析(独立的零依赖包,本模块绝不能 import);声明键材料的派生约定——每个键路径一个 purpose 串(`pkgcore.BootstrapKeyPurpose`),配合 `dbkit.DeriveBootstrapKey`——也在声明与工具箱侧。混两者会掉进"数据库连接串存在数据库里"的鸡生蛋死结:引导值按定义运行期不可变;动态值存在的意义恰恰是被改。
...
模块的接线拆成两步,拆分本身承重。**Register** 声明永远不需要已组装注册表的东西——审计 purpose、两条路由路径、变更事件、审计动作。**Attach** 要等每个组件的 `Init` 轮次都跑完才完成——描述符从它的 `Start` 步骤挂接并发布服务,而 Init 阶段消费者需要服务的宿主会在 `Init` 期间自己挂接,快照最终补全为同一集合——因为服务的 schema 是从注册表的*合并*条目与开关声明折出来的——别的模块拥有的条目是它的头等公民,schema 不可能在声明轮次完成前存在。Attach 要求注册不能碰的东西:已迁移的 `*gorm.DB`;只要有任一注册条目是 `Sensitive` 就必须有 cipher(否则 `ErrCipherRequired`——没有密钥的宿主读写这类值必然泄露);以及轮询间隔。它带幂等守卫(`ErrAlreadyAttached`),服务在 Register 与 Attach 之间的窗口回答 `config.service_not_attached` 而非空指针崩溃——宿主接线 bug 以编码错误浮现,绝不崩。
