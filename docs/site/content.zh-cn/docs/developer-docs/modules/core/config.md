...
- **引导解析不是本模块的事。** 如何够到基础设施——DSN、地址、部署模式、组合配置、引导密钥材料——在进程启动时一次性决定:键声明在每个组件的 `BootstrapKeys` 上、由通用加载器 `pkgcore/config` 解析(独立的零依赖包,本模块绝不能 import);声明键材料的派生约定——每个键路径一个 purpose 串(`pkgcore.BootstrapKeyPurpose`),配合 `dbkit.DeriveBootstrapKey`——也在声明与工具箱侧。混两者会掉进"数据库连接串存在数据库里"的鸡生蛋死结:引导值按定义运行期不可变;动态值存在的意义恰恰是被改。
