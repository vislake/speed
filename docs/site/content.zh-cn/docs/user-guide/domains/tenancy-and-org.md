...
解析器是宿主提供的函数:`tenancy.NewDomainResolver` 通过宿主自己的
lookup 函数按主机映射租户——某个 Host 是租户的自定义域名还是平台的
子域名,判定完全在 lookup 手里,参考应用的 lookup 就是一张部署配置
的扁平 host→租户映射表——未命中时回退到构造时配置的那个默认租户
(参考应用留空,于是未命中的主机读平台默认层);解析失败对登录页端
点仍以 200 提供该回退值,绝不出错。请求的租户从哪来是框架的事:你
的处理器用 `pkgcore.TenantFromContext(ctx)` 从上下文里读,绝不接受
来自头、参数或请求体的租户。
