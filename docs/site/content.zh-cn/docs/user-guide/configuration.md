...
配置文件、目标结构体上已设的默认值。键名由目标结构体字段派生:字段
...
**非密钥**开发默认值,真实部署必须从密钥库覆盖。平台为它们提供统一的派生
入口:`config.DeriveBootstrapKeyMaterial(rootKey, keyPath)` 按声明的键路径从
一枚 32 字节根密钥派生材料(目的串内嵌键路径,如 `speed.config.master_key.v1`);
宿主自行为根密钥与每个单键选择变量名,平台只接收字节。显式配置的单键始终
胜过为其派生的值;重命名声明的键路径即轮换,必须按轮换流程发布。零声明是
诚实状态:不消费启动输入的模块不声明任何键。
