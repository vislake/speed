...
真实的数据表与双语迁移,以及声明的权限、审计动作、事件与 job
handler;除 metering 外,每个模块还挂载 `/api/v1/*` 的 OpenAPI 片段
——metering 是按设计没有 HTTP 面的 Go 级 API,由业务模块进程内调用;
参考应用是每一个模块的强制首位消费者。
...
基础设施模块的 `ComponentRegistry`、一个供异步工作使用的 `jobs.Queue`——
