...
- **语言宣告,逐尝试读取。** 可选的 `languageProvider` 供给请求的
  `accept-language` 值——宿主返回它 i18n 实例的当前语言——与令牌
  一样逐尝试读取,所以语言切换之后发出的重试宣告新语言。调用方
  自带的 `accept-language` 绝不覆盖;null 或空串让该头缺席。它是给
  请求方语言内容(短信验证码的语言档)用的传输通道,绝不是响应本
  地化——响应始终是 code + params。
