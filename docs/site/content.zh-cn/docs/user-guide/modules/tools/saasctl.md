...
require 行。`--version` 必填(目前还没有任何发布物,目标版本永远
不会由工具发现)并按发布版本号语法校验。结果在写回前会从字节重新
解析并做结构自检;自检还会拒绝用 `replace`/`exclude` 抵消重写结果
...
接缝是点名给所有者的第一个任务。在第一个发布之前,生成项目带着
过渡态——require 停在占位版本、`replace` 指令指向 checkout、
`go.sum` 要等第一次消费方 `go mod tidy` 才会生成——而 `upgrade`
就是第一个发布被消费时走的那条机制。
