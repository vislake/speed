...
- 生产 PG 环境额外验证 RLS 生效：手工构造一条绕过 Go 层的查询，断言返回为空

**2. 双形态矩阵（与双方言矩阵正交，都要跑）**
- CI 对每个模块跑两遍测试：`SPEED_PROFILE=demo`（内存实现，快，无外部依赖）与 `SPEED_PROFILE=production`（testcontainers 拉 PG + Redis）
- 关键语义一致性断言：同一组用例在两种形态下，配额计数、事件投递顺序、计量汇总结果必须一致（这是防止两套实现语义漂移的唯一手段，也是这套抽象最容易出问题的地方）
- reference-app 的 e2e 在两种形态下各跑一次
- 验证 fail-fast：`profile=demo` + 多副本配置时启动必须报错退出
...
- 各渠道用官方沙箱或 mock server 覆盖授权码换取流程；demo 形态提供 `MockSocialProvider` 让本地无需真实应用凭证即可跑通
...
**11. 异步任务、媒体与集成**
- 任务队列：重试与指数退避生效；幂等键下重复提交只执行一次；worker 内租户上下文正确重建（构造一个不带 tenantctx 的任务，断言 Repository fail-closed 而非跨租户读取）；demo 形态下重启后未完成任务能恢复
...
- 站内信：未读计数准确；SSE 在 production 多实例下能正确扇出（起两个实例，验证连到 A 实例的客户端能收到 B 实例产生的通知）
...
- 生产 PG 下用受 RLS 约束的角色执行跨租户查询必须返回空，切到 `BYPASSRLS` 角色才可见
