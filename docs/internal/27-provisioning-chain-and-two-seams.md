...
- **E 进 `hostcore` 共享内核**:hostcore 是"宿主中性内核"(存活探针、预认证允许列表、挂载标签、生命周期),以逐字节双副本方式共享(examples/reference-app 与 saasctl 模板的 `internal/hostcore/hostcore.go`,由 `tools/check_host_core_parity.py` 钉住)。供给链是策略 + 模块胶水,把 org/rbac/billing 的 import 拉进一个必须逐字节一致的宿主中性文件,既违 [14 示例应用](14-reference-app.md)"只属于一个宿主的行为不进内核"的边界,也让内核失去中性。
