...
| 组织树 + 前缀匹配的策略规模与性能（判定引擎已改为自建，见 [05](05-identity-and-access.md) 的「实现落地更正」） | M1 早期打样并压测；策略缓存 + 按租户懒加载；节点数超阈值时告警 |
...
| `dbkit.Repository[T].Delete` 曾只有物理删除一种语义——2026-09 该缺口已在 [04 数据层与多租户隔离](04-data-and-tenancy.md) 的"删除语义"一节补齐设计，并分两轮落地为代码：软删除轮次落地 `SoftDeletable` 标记接口与 `Repository[T].Restore`（实现该接口的模型 `Delete` 变为标记删除、未实现的模型字节级不变），硬删除轮次（2026-09-03）落地系统上下文门禁的 `Repository[T].HardDelete`（`hard_delete.go`：普通租户上下文在触碰数据库之前即被拒绝、门禁通过后租户仍绑定不越界、软删除行同样可物理擦除、RLS 集成测试覆盖跨租户不可擦除性质）。机制缺口已关闭，残余暴露在**编排层**：按租户可配置的保留窗口、`jobs` 定时把过期标记删除行转 `HardDelete`、"被遗忘权"受理流程都未落地；参考应用 notes 模块的消费证明是服务级的，没有 HTTP delete/restore 端点。 | 机制层承诺已兑现且向后兼容；编排层——保留期配置、`jobs` 清理任务、`compliance` 被遗忘权流程（均改为调用 `HardDelete`）——属于 `compliance` 模块（M4）治理层，落地前不得把"标记删除 + 到期自动彻底删除"包装成对外承诺的能力。 |
