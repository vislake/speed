...
| `dbkit.Repository[T].Delete` 目前只有物理删除一种语义，没有软删除：终端用户手滑删错数据后无法自助找回，只能走工程/客服人工兜底；同时合规要求的"彻底删除"目前和普通删除是同一条代码路径，无法区分"用户删的"与"合规擦除的"，也没有保留窗口。2026-09 已在 [04 数据层与多租户隔离](04-data-and-tenancy.md) 的"删除语义"一节补齐设计（标记删除/彻底删除两条独立入口、`SoftDeletable` 标记接口、`HardDelete` 走系统上下文白名单），但设计尚未落地为代码，`Repository[T].Delete` 现状不变。 | 按设计新增 `SoftDeletable` 标记接口与 `Repository[T].Restore`/`HardDelete`，未实现该接口的模型保持现有物理删除行为不变，向后兼容；`jobs` 保留期清理任务与 `compliance` 被遗忘权流程改为调用 `HardDelete`；落地轮次待定，落地前不得把"标记删除"包装成对外承诺的能力。 |
