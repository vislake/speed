...
- **多套实现**：字节所在的 `ObjectStore` seam 的本地 FS 与 S3 兼容双实现已由 `pkgcore` 落地（见 [03 部署模式](03-deployment-modes.md) 与 `go/pkgcore` 的 census 条目），storage 自身只依赖 seam 接口；本模块的集成层以 PostgreSQL + RustFS 两条腿验证（`go test -tags=integration`）。
