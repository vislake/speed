...
- **两种部署模式均有实现**：单进程部署模式 = 进程内 worker pool + SQLite 持久化任务表（重启后能恢复，不是纯内存——任务丢失比配额少算严重得多）；分布式部署模式 = Redis（`hibiken/asynq`，成熟且自带重试/延迟/定时/Web UI，不必自造）。
...
- **两种部署模式**：单进程部署模式 = 本地文件系统 + 应用自身提供带鉴权的读取端点；分布式部署模式 = S3 兼容（MinIO / 阿里云 OSS / AWS S3）。
...
- **实时推送**：SSE 通道（比 WebSocket 轻，且这里只需服务端单向推）。两种部署模式：单进程部署模式下直接推；分布式部署模式下经 Redis Pub/Sub 扇出到所有实例。不做实时的话前端只能轮询，体验和成本都差。
...
- 两种部署模式：单进程部署模式 = 控制台输出 + 站内信落库（站内信在单进程部署模式下完全可用，无需任何外部服务）；分布式部署模式 = SMTP / 短信网关 / 推送服务。
...
- **限流三层**：全局、租户级、Key 级；算法用滑动窗口计数（基于 `KVStore`，两种部署模式天然支持）。超限返回 `429` 并带 `Retry-After` 与配额响应头。
