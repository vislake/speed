...
- **一切非单元测试类进入专有目录,绝不混入源码目录**:真实后端集成 → 每个 Go 包的 `integration_test/`;浏览器端到端 → `e2e/`(Playwright);应用的组合 HTTP/装配流 → 专用的应用级测试目录;跨实现契约套件 → 该模块自己的支撑包(`queuetest`/`eventbustest` 模型);迁移套件 → 紧邻所练的迁移集。
...
仓库里 `*_conformance_test.go` 命名的共五个;共同形态:它们都已是**外部测试包**(`pkgcore_test`/`jobs_test`)的薄驱动——真正的共享契约套件早已住在各模块的支撑包,驱动文件只因"内部测试文件导入会回导 pkgcore/jobs 的支撑包会成环"而被迫外部化(文件头自述这是既有布局规则认名的机械例外)。
...
2b 只移动了文件本身;仓库散文对这批套件的旧 `cmd/server/<文件>_test.go` 归属的引用仍残留 102 处(95f9bb13 上实测:落于 98 行、跨 41 个文件),分布在根 CLAUDE.md 普查、模块 AGENTS/README、`docs/internal` 各实现对照、reference-app 内部包注释与 web 侧测试工具注释里。本记录逐项重定向到 `flowtests/`(全仓库 `cmd/server/<47 个迁走文件>` 前缀引用为零命中):
