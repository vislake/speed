...
- 输出与字节幂等(main.go 的 `renderOutputs`):仓库根 `docs/config-reference.md`、`docs/config-reference.json`、根 `.env.example`、`config.example.json` 与站点页 `docs/site/content.en/docs/user-guide/configuration.md`(根 `.env.example` 头注明为 configrefgen 所生成、键缺漏会触发 drift 门;步骤二落地后该产物随宿主面一起移除,产物集为四件,§9)。`--check` 即漂移门(归位后为工作目录 `tools/configrefgen` 下 `go run . --check`;configrefgen 自带单测)。
...
- 站点与文档链:docs/site 下 content.en 与 content.zh-cn 对称,2026-09-10 按 `APP_|config-reference` 对两语区实跑 grep 各命中 8 页——`config-reference` 字面在站点零命中,8 页全部因 `APP_` env 字面命中(`SPEED_|os.Getenv|ConfigFromEnv` 复查面仅另见 i18n 页的 `SPEED_LOCALE_STORAGE_KEY` 与 observability-and-ops 页的 `OTLP_ENDPOINT` 直读示例,均与引导链无关,不入清单)。命中页按步骤二是否需人工核对分两类:
  ...
  站点外的按名消费载体:`examples/reference-app/README.md`、`DEPLOY.md` 是操作文案载体;`docker-compose.yml`、`docker-compose.distributed.yml`、根 `fly.toml` 与双副本集成测试(用 `APP_DEPLOYMENT_MODE=distributed`、`APP_DISABLE_QUEUE_WORKER=true`;脚手架验证以 distributed 模式起生成骨架)按名消费 env。
...
- configrefgen(现于 `tools/configrefgen`,见 §5.2 迁移注记):渲染改造(模块声明 → per-module 键清单;宿主 29 键过渡期仍走 curated 表,两源对账门:键/名/分组不重不漏,表头注明过渡态;同时实现 §2 防线 2——运行时项键与 bootstrap 键交叠即红)、`.env.example`/JSON 对齐、站点页与 `config.example.json` 同入产物集(步骤一即已渲染)、漂移门不动(--check 即门);
...
验证期望:受影响模块 `task test` 全绿;`configrefgen --check` 通过且输出重生成提交;各仓库检查脚本与 lint 绿;golangci 零新告警。
...
验证期望:步骤一全部项 + flowtests 全绿;`go run ./cmd/server` 零 env 手动冒烟(standalone/SQLite 照常起);双副本 flowtests 过;站点构建零告警。
...
- Q1 步骤二之后,`saasctl` 生成骨架(模板 `cmd/server/config.go`,20 键)与 `appconfig` twin 是否另起一轮跟进 loader?骨架是"可自由编辑"的生成产物,维持直读 env 有可辩护性;但消费者骨架若永远直读,loader 机制在参考应用之外的第二个证明面就缺失。建议:独立后续轮评估,本两阶段不动。**已由 §9.7 落地:骨架跟进 loader(行为等价的加载层迁移),不采用根密钥派生;闭:`21a656c5`、`31d0c1e3`(2026-09-10)。**
...
- Q4 **步骤一落地时的版本钉子**:普通语义下"模块同轮消费新 pkgcore API"需要该 API 已发布——`Registry.Bootstrap`/`BootstrapKey` 在两阶段设计写就时尚未进入任何已发布版本,而 v0.0.1 之后的模块 `go.mod` 已按发布后的形态钉真实 tag(`GOWORK=off go build` 因此会红)。伪版本不是出路:v0.0.1 已在代理上,v0.0.1 之后的提交在 MVS 里排在 `v0.0.1` 之下,`go mod tidy` 会把钉子升回已发布版本,模块拿不到新 API。因此本轮对五个声明模块(authn、org、notification、pki、config)采用仓库此前的过渡形态——`replace github.com/vislake/speed/go/pkgcore => ../pkgcore`(依赖方的 replace 对消费者无效,只影响模块自身的独立构建)。**第六个 replace 随 §9.6 的 L2 轮加入:`go/dbkit`**(`DeriveBootstrapKey` 组合席位 API `pkgcore.BootstrapKeyPurpose`,理由与形态同上)。下一次 lockstep 发布需执行同类清理:删除这六行 replace,把 require 提升到与新平台同版发布的 tag(与 `first_release_replace_cleanup` 同一清点方式);漏做则该模块的新代码在本仓库之外不可用,因为消费者会按 require 里的旧 tag 解析 pkgcore。这个清理随**下一个版本号的发布**执行,不能寄望 v0.0.1 重发:v0.0.1 的 21 个模块 tag 已从远端与本地删除,而 Go module proxy 对已服务过的版本不可撤回——缓存仍在且内容不可改写,删 tag 只让 v0.0.1 在仓库里不可再生,代理上那份依旧按原样解析,因此该版本号已经作废,既不能再发布也不能被新内容复用;下一次发布必须换一个新版本号,replace 的删除与 require 的同版抬升随之落在该版本上。此后按同一形态追加:因消费未发布的 `apperr.HasCode`、pkgcore 事件载荷助手与 `dbkit.IsRecordNotFound`,十一个模块新增 19 行模块级 replace(10 行指向 dbkit、9 行指向 pkgcore;逐模块理由见各 go.mod 的注释)。发布清理动作不变:清空全部仓库内 replace、require 随新版本抬升。
...
- **生成器换源**:`bootstrap.go` 只保留两层互斥规则(`overlappingKeys`);`document.go` 的 bootstrap 节渲染"机制陈述 + per-module 声明表 + 零声明条款";`--check` 产物四件;新增渲染键集 ↔ 声明集双向测试。覆盖说明随参考面改写(配置参考只看平台面),而错误码索引的扫描面加宽为 `examples/reference-app/internal/**`(同时补上 `internal/cases`、`internal/notes`、`internal/smilesim` 的既有缺口),`docs/site/static/llms.txt` 的配置参考描述同步。
- **设计文档维护**:§1、§4、§5 内凡"文件:行号"式引用一律改为符号名优先(行号随步骤一/二落地已失效)。

### 9.3 有意行为差异与本轮未做

- 四枚 int/bool 键的空串收紧(§9.2)是设计裁定的有意行为;此外无行为差异。
- 未做(与本设计无关或按既定排期):saasctl 生成骨架与 `appconfig` twin 维持直读(Q1);五个声明模块的过渡 `replace` 随下一个版本号的发布清理(Q4)。`examples/reference-app/.env.example` 已覆盖全部 35 键,并由 env-example 一致性检查器(`tools/check_env_example_consistency.py`,自带 planted-drift 套件)与 loader 形状结构体的 `config:"env=NAME"` 钉及 `APP_ROOT_KEY` 读点持续对齐。
...
用户裁定(2026-09-10)与 §9.4 末条同工落地;本节的迁位与删除两项即该项裁定的配套裁定:

- **示例迁位**:手写源 `config.example.yaml` 与其派生产物 `config.example.json` 从仓库根迁入 `docs/`(`docs/config.example.yaml` / `docs/config.example.json`,成对相邻)。随迁面:生成器的产物路径(`tools/configrefgen/outputs.go`)与手写源/加载路径常量、全部测试加载路径、`.gitleaks.toml` 的路径作用域、三处指路句(md 导言、站点 EN 生成页、站点 zh 手写页);四产物重生成,`--check` 绿。
- **宿主演示块删除**:平台示例中的宿主自有键演示小节(deploymentmode / port / dbpath / redis.addr / smtp.*)与指涉它的头段一并删除——平台示例零宿主键影;余下机制散文(四源优先序、env 双下划线拼写与单下划线不构成嵌套的规则)的示例改用平台键自身(如 `authn.blind_index_key`)。四源 pin 测试改以平台六键为载荷运行(旗标 > env > 文件 > 结构体默认全序、双层嵌套、单下划线不解析、文件缺席时结构体默认成立),生成器侧的演示宿主结构体(DeploymentMode/Port/DBPath/Redis/SMTP 字段)退役——生成器内与示例内不再有任何宿主名 fixture;`docs/config-reference.md` 导言句去掉演示半句。

### 9.6 loader 集成根密钥派生(L2,2026-09-10 用户立项)

用户裁定后实施:派生不再由宿主两调用自行组合,而是集成进 `pkgcore/config` loader——宿主退化为"指一个根密钥来源 + 字段打标记"。落地形态:

- **五源链**:`flag > env > file > 派生 > 结构体默认`,派生层仅作用于打 `derive` 标记的字段;非派生键仍是四源。`derive` 是裸 token(`config:"env=NAME,derive"`),仅限**恰好 `[]byte` 型**字段(否则 `ErrInvalidTarget`),重复 token 同拒。
- **不进文本管线**:`Load` 新增 `applyKeyMaterial` 步——显式值(须 64 hex,否则 `ErrInvalidValue` 点名键与来源;非文本值同样拒绝)hex 解码 > 根密钥派生 > 不动;派生字段的键在处理后从 koanf 输入移除,值一律以新 slice 反射写入(派生结果先 clone)。**别名守护(实测风险规避)**:mapstructure 的 `decodeSlice` 对非 nil 目标 slice 就地写 backing array——derive 字段若预填宿主包级 `Dev*` 常量并走文本管线,该常量会被 hex 文本静默改写;loader 侧与应用侧各有断言 `Dev*` 逐字节不变的守护测试(扰动实证:去掉该步即红)。空串/nil 值按源丢弃=未设(与"空值=未设"逐字一致)。
- **API**:`WithRootKey([]byte)`(nil/空=未配置;非 32 字节 → `ErrInvalidRootKey` 点名该选项)、`WithRootKeyEnv(name)`(**必装项**:reference-app 扫描门禁 `os.Getenv`,根密钥须与派生字段在同一次 Load 消费;未设/空=未配置,配置坏值 → `ErrInvalidRootKey` 点名变量;空名 panic)、`WithKeyDerivation(fn)`;第二个 root-key 选项任意组合 panic(两个 secret 无"后者胜"语义)。有根密钥、有派生字段而未装 deriver → `ErrInvalidTarget`。新哨兵 `config.ErrInvalidRootKey`(plain,boot 期库面,不进 apperr/错误码索引)。**派生身份 = 字段键路径**(重命名字段=轮换,写进 godoc 与 AGENTS)。
- **依赖方向**:loader 收无名函数类型(零新 import,零依赖招牌不变),宿主一行接入 `WithKeyDerivation(dbkit.DeriveBootstrapKey)`;dbkit 新增 `DeriveBootstrapKey(rootKey, keyPath)` 适配器(`pkgcore.BootstrapKeyPurpose` + `DeriveKey` 的组合),函数类型结构化、不 import `pkgcore/config`。HKDF 下沉 pkgcore 被拒(Go internal 规则会逼出公共 API、错误身份分裂、pkgcore/config 被迫 import pkgcore 根、迁移面大而无收益)。代价:dbkit 加 `replace pkgcore => ../pkgcore`(见 Q4,现共六行)。
- **reference-app 迁移**:三级逻辑整体消亡——`resolveKey`/`parseHexKeyEnv`/`configKeyHexLength`/`RootKey` 字段及宿主键清单中的 `rootkey` 行全删;六枚字段 `string→[]byte` 加 `,derive`;六个 dev 默认上移为结构体默认(复用 `Dev*` 变量);`loadHostConfig` 装两选项(`WithRootKeyEnv("APP_ROOT_KEY")` 不变宿主变量名 + `WithKeyDerivation(dbkit.DeriveBootstrapKey)`)。四个 flowtests 语义测试**逐字**通过(验收核心,零修改);等价性 oracle 冻结为迁移前版本、同样零修改;两处错误文案断言放宽为行为/哨兵断言(`ErrInvalidValue`+点名变量、`ErrInvalidRootKey`+点名 `APP_ROOT_KEY`)。
- **测试与示例**:loader 单测覆盖全序/无根密钥/空值与 nil/长度与 hex 错/缺 deriver/文件标量非文本(**YAML 会把全数字 64 文本读成数字,文件值须引号——已入文档与错误文案**)/派生失败与长度错/派生键路径身份/panic 规则/来源点名/slice 别名守护;Example 用替身 deriver(**示例包不得 import dbkit**,破除方向)。dbkit 适配器测试以冻结字面量 purpose 钉组合。
- **文档面**:`pkgcore/config` 包文档(五源 + Key material 一节)、`pkgcore` AGENTS(loader 小节 + `ErrInvalidRootKey` 错误行)、`dbkit` AGENTS(文件表行 + 适配器条目)、reference-app `.env.example`/`DEPLOY.md`/`README`、`configrefgen` 机制句("Precedence is the host's to apply" 改为 loader 施加)与四产物重生成、zh 站点配置页、`11-cross-cutting.md` 优先级链补派生层、本 §9.6 与 Q4 第六行 replace。

### 9.7 saasctl 生成骨架跟进 loader(Q1 落地,2026-09-10 后续轮)

Q1 建议的独立后续轮已落地:生成骨架不再直读 env,`os.Getenv` 从模板可执行代码归零。落地形态,以及两处有意偏离:

- **加载形态**(`go/saasctl/internal/template/project/cmd/server/config.go`):新增 loader 目标结构体 `hostConfig`——21 个字段全部以 `config:"env=…"` 钉住确切变量名(骨架变量是平铺单下划线拼写,loader 的嵌套派生名对不上),`DeploymentMode`/`Port`/`DBPath` 三枚标量默认由 `hostConfigDefaults` 预置(loader 第四源);`loadHostConfig` 以 `config.New(config.WithEnvPrefix("APP_"))` 驱动(骨架无根密钥语义,故不装派生选项);`configFromEnv` 退化为"加载 + transform",transform(`serverConfigFrom`)保留原实现全部解析规则:部署形态解析、五枚密钥的 hex 校验(`parseKeyEnv` 原样保留,错误文案逐字不变)、文本→int/bool 解析(含各自错误文案)、S3/SMTP 完备性拒绝。**字段类型刻意全为 string**:空串即"未设"是骨架全键共享的契约,string 字段在 loader 下对该语义逐字保持(§9.3 参考应用接受的 int/bool 空串收紧不在骨架采用);行为等价由 twin 对账测试与真实物化构建证明。
- **OTLP 与导出器接线**(随行闭合 `go/observability/AGENTS.md` 记录的模板缺口):`cmd/server/main.go` 空白导入 `exporter/otlp` 与 `exporter/prometheus`,新增第 21 个变量 `APP_OTLP_ENDPOINT`(target 字段 + `serverConfig` 字段);端点非空时以 `obs.WithOTLPEndpoint` 条件接线(参考应用同形)。未设时导出器与 `/metrics` 语义升级为真实抓取输出(此前模板缺 prometheus 空白导入,记录在案);设了端点则 `/metrics` 设计性 404(无本地 registry)。
- **twin 关系**:`internal/appconfig` 仍是手写孪生——模板文件是 `//go:build ignore` 资产、loader 不暴露逐值来源而 `config print` 的 provenance 正需要它;对账测试从"提取 `*Env` 常量声明"改为"提取 `config:"env=…"` 标签",并新增钉面:五枚密钥的 `parseKeyEnv("APP_…", encoded)` 字面量与标签同名、解析顺序(transform 内 `hc.<字段>` 首次出现序,与 twin 的 Load 序一致)、错误格式串与 dev 密钥字节继续逐字钉死。
...
- **顺序确定性**:文源层无书写序(koanf 成图即失序),按名排序;代码覆盖层保序(`ComponentConfig.With` 调用序);写作当时的过渡适配器即靠它复现 WithModules 的模块序(该过渡面后已退役)。
