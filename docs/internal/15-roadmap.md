...
**关于 `FileUploader` 的排期（2026-09 更新）**：M2 行内容单元格列出的 `ui-kit` 的 `FileUploader` 已作为独立组件轮提前于 M2 计划窗口落地（紧随 storage 模块轮）。已交付：受控队列组件——pick 队列与每行传输状态（上传中/成功/失败，重试/取消/移除）作为 interaction-local 例外存于组件内（与 `ConfirmDialog` 的 armed 状态同类，是「组件全部受控」纪律的具名例外），每次队列变化经 `onQueueChange` 上报；上传本身由 host 注入的 `execute(file, { signal, onProgress })` 逐文件执行、可中止，**组件零网络**——大小/类型/数量预校验与并发上限是 executor（host）的职责。消费证明：组件行为套件（fixture executor 覆盖全部交互状态）+ README quick-start 的脚本化 fetch 上传面板（`src/usage-example.test.tsx`）；浏览器 + 真服务器 leg 仍属 consumer shell。storage 的 api-sdk 前端腿不随本轮恢复——仍按 storage 轮自己的 deferral 推迟到 consumer-shell round，wire 契约权威现为 `go/storage/api/openapi.yaml` 本身；延期三方互指（`go/storage/AGENTS.md`、Taskfile `api:gen` 头部注释、ui-kit AGENTS.md 的 deferral 条目）与本条共同记录。M2 行其余单元格与上一条 storage 注记不变。
