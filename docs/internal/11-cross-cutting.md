...
|---|---|---|
| 内容 | 数据库连接、Redis 地址、部署模式、监听端口、加密主密钥、日志级别 | 功能开关、套餐默认限额、SMTP 参数、AI 模型默认参数、品牌信息、可用支付渠道 |
...
- **热更新**：变更后经事件总线广播 `config.changed`，各实例刷新本地缓存；同时带 TTL 兜底轮询（防止事件丢失）。单进程部署模式下用内存总线（同进程内天然即时生效），分布式部署模式下用 Redis——复用已有机制，不引入新组件。
...
**算法：滑动窗口计数器近似，而不是滑动窗口日志。** 这个选择由 `KVStore` 的实际契约决定，不是随意挑的：`KVStore` 故意不提供服务端脚本、pipeline，也不提供超出"不透明字节值"之外的数据类型——这样单进程部署模式的内存 map 与分布式部署模式的 Redis（或等价实现）才能同等地实现它。滑动窗口日志需要为每次请求单独记一条时间戳、再按窗口范围查询，这需要有序集合一类的结构，`KVStore` 的契约给不了；滑动窗口计数器只需要"计数 + 一个界定窗口的 TTL"，`KVStore` 现有的两个原语正好够用：
