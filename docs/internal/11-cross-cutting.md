...
- 优先级链固定为：**命令行 flag > 环境变量 > 配置文件 > 派生 > 内置默认值**，写进文档不允许各模块自行发挥。派生层是 loader 的第五源，仅作用于打 `derive` 标记的 `[]byte` 密钥材料字段（`pkgcore/config` 施加，材料由宿主经 `WithKeyDerivation` 装入的平台组合产出）；其余键仍是四源。
