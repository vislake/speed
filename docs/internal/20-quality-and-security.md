...
- **覆盖率不允许下降**：与入库基线比对（容差 0.15 个百分点，按实测负载敏感抖动上沿设定——0.05 时期按六个地基模块约 0.03 的实测上沿；扩展到全部模块后，go/authn 的 MFA 并发确认竞速测试暴露约 0.12 的上沿，容差随之上调，详见下方注记）——先于下限存在的地基模块规则，保留至今：测得的覆盖即使仍高于 80%，只要比基线低得超过容差同样失败，防止覆盖率在高位缓慢滑落
