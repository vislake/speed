...
- **登记理由**：破坏面登记（三处公共符号移除，加一处行为收紧——组装之外不再解析 provider 名）。本批四个破坏面提交（`cd9c7a8b`/`0312806d`/`ab0b6cd7`/`c60a0610`）带 `!BREAKING` footer，随批的测试/文档提交（`b5fd155a`/`18804e4a`/`7081366b`）不带 footer（未触公共面）；§9 的「29 §9 目录式组件收编」挂起项随之闭合。
- **出处**：`cd9c7a8b`（`feat(pkgcore)!: remove the seam registry machinery`）+ `0312806d`（`refactor(ai-gateway)!: resolve providers through the component face only`）+ `ab0b6cd7`（`refactor(pki)!: retire the signer registry`）+ `c60a0610`（`refactor(billing)!: retire the payment gateway registry`）+ `b5fd155a`（`test(reference-app): assemble provider fixtures through the component face`）+ `18804e4a`（`docs: narrate the resolution face as components, not registries`）。
