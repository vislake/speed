...
- **替代路径**：注册表的声明席位访问器（`reg.ConfigSeat()`、`reg.EventsSeat()`…）；`go/pkgcore/componenttest` 提供测试侧窗口帮手。路由与中间件两个 HTTP 声明面此后随 http 组件下放为组件产物（见 §5 对应条目）。
...
### 5.31. pkgcore：ConfigSchema 默认命名空间翻转为裸路径、`NoConfigNamespace` 哨兵删除（破坏面登记）
...
- **出处**：`be0770b1`（`fix(pkgcore)!: resolve schema fields at their bare paths by default`）+ `3fca1623`（`fix(app): mirror the bare-path default in the component config resolver`）+ `ac4982c2`（`docs(pkgcore): state the configuration contract at its bare-path default`）+ `1f3ae6ec`（`docs(authn,config,notification,org,pki): restate the namespace rationale`）。

### 5.32. pkgcore：宿主覆盖辅助升格为导出 API（非破坏，新增面登记）
...
### 5.33. go/observability：observability 组件改由其自身包声明（非破坏，组装面登记）
...
### 5.34. pkgcore：平台中间件声明席（第十一席，跨 go/app 与 go/observability 落地）（非破坏，新增面登记）
...
### 5.35. pkgcore 与两模块：目录式交付契约（`ProvidesMember` / `Catalog`+`MinMembers` / `Member`+`Members`）落位（宿主可见面登记）
...
### 5.36. pkgcore 与 go/app：Serve 阶段、阶段只读读法与 Stop 两拍（组装契约登记）
...
- **出处**：`cb7510f9`（`refactor(pkgcore)!: retire the HTTP declaration seats from the registry`，带 `!BREAKING` footer）+ `13fe2d62`（`feat(pkgcore): add the route-mount helper and the face recorders`）+ `6be083d3`（`feat(app): add the http component, the process's HTTP face`）+ `6824d666`（`refactor(app): derive the fixed chain from the component's route source`）+ `533e490b`（`refactor(authn,org,pki,storage,sharing,notification,integration,admin,billing,ai-gateway,config): mount routes through the optional route face`）+ `4fe91af2`（`refactor(observability): declare the tracing layer on the http component's middleware face`）+ `f64d8f0f`（`refactor(reference-app): hand the listener to the http component`）+ `7a7547e2`（`refactor(saasctl): hand the generated host's listener to the http component`）+ `d4e7c31f`（`chore(tools): hold the http component to the host-composition gate`）+ `4f8ad6b6`（`docs(internal): record the http component and the seat downshift`）+ `678813fa`（`docs: align the AGENTS files, the site pages and the skill with the http component`）。
...
- **状态**：e2e 套件已由 stub 改为真套件（含诚实门）；"e2e 仍是 stub" 类过时表述已清扫，docs/internal 同族句一并对齐。
