...
| 事件总线 | `EventBus` | 进程内 channel / Redis Streams（消费者组）/ NATS JetStream（`go/pkgcore/eventbus/nats`，每副本一个唯一命名的持久 pull consumer，同一投递语义：广播到每个副本，而不是消费者组式的负载均衡） |
