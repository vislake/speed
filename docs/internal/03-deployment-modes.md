...
| **裸注入**（`WithEventBus(...)` 等） | 测试 double、一次性对象、共享资源（如 KV 与 EventBus 共用一个 `*redis.Client`）、以及不想经名字体系的情形 | 直接注入；注入恒优先于 preset（按 seam），内核不关闭宿主给的对象 |

**名字注册路径的落地形态**：每个分布式实现包在内置注册之外导出自己的 `Registration(name, <typed 构造输入>)` 工厂（redis/nats/memcached 收 client/conn、postgres 收 pool（eventbus 侧另收 replicaID）、s3 收它的 typed `Config`），并导出该实现声明的 `Capabilities` 常量——内置注册与工厂引用同一个常量，宿主手写注入实参时也照抄它，声明值与装配校验值不可能漂移。宿主的用法：自建对象（TLS 客户端、Sentinel/Cluster 拓扑、凭据链解析结果、跨 seam 共享的资源）→ 工厂包成 `Registration` → 以自定名字 `Register`（内置名已被占用，重名报 `ErrDuplicateImplementation`）→ `Preset` 条目引用该名字。条目里的 `Config` 被工厂忽略（对象已带齐一切设置），`New` 返回裸实现值、不带 closer，`Kernel.Shutdown` 不碰宿主对象——与 `With*` 注入同一条所有权契约。部署切换因此保持纯配置：换部署只改 `Preset` 条目里的 `Implementation` 字符串，typed 对象始终留在宿主手里。根包内建的 memory/console/local/smtp 不设工厂也不导出常量：它们的全部设置都可由扁平字符串表达（配置通道已覆盖），没有需要工厂携带的 typed 输入。端到端证明在 `go/pkgcore/unittest/factory_registration_test.go`（一个共享 client 走两个 seam 的完整回路：注册别名 → preset 引用 → `Bootstrap` 解析 → 经解析值触达宿主 client → `Shutdown` 不动它）。
