...
### 这个取舍的代价必须被量化，它的适用边界必须被检验

上一段的"代价"过去只是一句定性描述。它可以精确测量——新建一个空模块、`require` 目标模块、`go mod tidy`（`GOWORK=off`），数 `// indirect` 条目即可。这个测量应当在**每次向内置清单添加实现时**重做一次，作为"逐个论证"的可检验部分，而不是凭感觉判断某个 SDK "还好"。

取舍的论证前提是**运行时可切换组装的价值**。这个前提不是普遍成立的，因此有三条边界：

1. **前提不成立的实现应当分包。** 论证成立于"同一个二进制换个 preset 就能改用另一套后端"。对本来就不会在运行时切换的实现，这个价值为零，代价却照付——数据库方言是典型：没有人会让一个已部署的实例从 SQLite 切到 PostgreSQL，那是重新部署，不是换 preset。这类实现内联进接口所在的包，是纯粹的净损失。

2. **接口的纯消费者不该为实现付费。** Go 的依赖解析按**包**而非按符号进行，因此"实现与接口同包"意味着任何 import 该包的模块都继承实现的全部依赖，哪怕它一个实现都没用到。这一条与"必须遵守的约束"第 1 条是不同层次的事，而且第 1 条覆盖不到它：一个模块可以完全遵守"业务代码只依赖接口，永远不 import 具体实现"，却仍然因为接口与实现同包而背上后端 SDK。真实例子是 `go/ratelimit`——一个建立在 `KVStore` 接口之上的滑动窗口计数器，非测试代码里零第三方 import，消费者却要为它承担 24 个 indirect 依赖，其中包含 S3 SDK。

3. **代价随依赖图向上累积，位置越靠底层越贵。** 底座模块内联一套实现，传染面是全部模块；叶子模块内联，只影响自己的消费者。因此越靠近 `pkgcore` 的位置，"逐个论证"的门槛应当越高。

违反这三条边界的现存实例、各自的实测数字与修复方案，作为独立议题在 issue 中跟踪，不在本文展开——本文只确立原则。
...
6. **向内置清单添加一套实现时，必须实测它给消费者带来的依赖增量并写进 PR**。约束 1 管的是源码层面（业务代码不 import 实现），这一条管的是打包层面（实现的依赖会不会落到接口消费者头上）——两者不能互相替代，因为 Go 按包解析依赖，同包内联的实现无法被消费者裁剪掉。测法与三条适用边界见"实现注册表"节。运行时不会切换的实现（数据库方言是典型）不适用内置清单的论证，应当分包。
