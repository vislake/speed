...
| 缓存 / 计数器 | `KVStore` | 进程内 `sync.Map` + TTL / Redis / PostgreSQL（单表 + TTL 列，`kv/postgres`，2026-09） / Memcached（`go/pkgcore/kv/memcached`——**无持久化**：Memcached 是纯内存缓存，进程重启、滚动升级或普通内存压力下的 LRU 淘汰都会无声丢光全部数据，`SurvivesRestart` 诚实地不声明；仅适合可从权威数据源重建的一次性状态，不适合会话状态或功能开关覆盖） |
