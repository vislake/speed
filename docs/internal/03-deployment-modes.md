...
能力表是可扩展的，扩展点已经实践过：`Stateless` 位（见上表）——横幅警告（约束 5）对 `mailer.console` 这类无状态实现是空洞的，于是加一个「无状态」位来豁免，警告只对有状态却不持久的实现打出。`KeyNeverLeavesBoundary`（见上表）的形状不太一样：它不描述四个内置 seam 中任何一个的状态，而是由 `go/pki` 自己的 `Signer` 实现通过同一套 `pkgcore.SeamRegistry[Signer]` 机制声明；`Kernel.Bootstrap` **不**对它做任何校验——`resolveKernelSeam`/`validateSeamCapability` 只认 `EventBus`/`KVStore`/`Mailer`/`ObjectStore` 这四个固定内置 seam，`pki.Module.WithSigner` 没有能力位要求参数，`SignerRegistry.Build` 只是把注册时声明的能力位原样交还，从不比对——所以一个宿主误装了一个不满足这个位的 `Signer` 到本该要求它的场景，不会报错（详见 `go/pki/AGENTS.md` 的 Known limitations）。将来若需要区分投递顺序保证、消费者组语义等维度，照旧是往表里加行，不是加开关。
...
- **逐项覆盖**＝从一个内置 preset 派生一份副本、只改要覆盖的条目（落地形态是 `Preset.With(seam, SeamPreset{...})`，返回副本，从不写穿内置常量）——这正是宿主配置层与内置 preset 之间的那一层。

三条路径各司其职，不互相替代：

| 路径 | 适用 | 形状 |
|---|---|---|
| **纯配置通道** | 该 seam 的设置可用扁平字符串表达（地址、凭据、桶名等），每 seam 独立连接，零装配代码 | `Preset` 条目带 `Config`；实现的 `Registration.New` 从 `Config` 自建资源，内核持有并关闭它 |
| **类型化名字注册** | 需要 typed/复杂配置（TLS、Sentinel/Cluster、凭据链）或要共享既有资源 | 宿主构建对象 → 包进 `Registration` 的闭包 → 以自定义名字 `Register` → `Preset` 引用该名字；对象所有权留在宿主 |
| **裸注入**（`WithEventBus(...)` 等） | 测试 double、一次性对象、共享资源（如 KV 与 EventBus 共用一个 `*redis.Client`）、以及不想经名字体系的情形 | 直接注入；注入恒优先于 preset（按 seam），内核不关闭宿主给的对象。`eventbus/redis`、`kv/redis`、`objectstore/s3` 各出一站式构造器（`FromAddr`/`FromConfig`）：把宿主原先手搓 driver client 的那一步收进实现包，一次调用返回「实例 + 该包自声明的 `Capabilities`」这对 `With*` 实参（**不做 preset 选择器**——选谁仍是内核 wiring 的决定）；两个 redis 包返回带 `Close() error` 的 owned 值（释放构造器自建的 client，宿主停机时自行关闭），s3 无 client 可关、返回裸 store；空地址/不可用配置返回错误而非 panic |

**名字注册路径的落地形态**：每个分布式实现包在内置注册之外导出自己的 `Registration(name, <typed 构造输入>)` 工厂（redis/nats/memcached 收 client/conn、postgres 收 pool（eventbus 侧另收 replicaID）、s3 收它的 typed `Config`），并导出该实现声明的 `Capabilities` 常量——内置注册与工厂引用同一个常量，宿主手写注入实参时也照抄它，声明值与装配校验值不可能漂移。宿主的用法：自建对象（TLS 客户端、Sentinel/Cluster 拓扑、凭据链解析结果、跨 seam 共享的资源）→ 工厂包成 `Registration` → 以自定名字 `Register`（内置名已被占用，重名报 `ErrDuplicateImplementation`）→ `Preset` 条目引用该名字。条目里的 `Config` 被工厂忽略（对象已带齐一切设置），`New` 返回裸实现值、不带 closer，`Kernel.Shutdown` 不碰宿主对象——与 `With*` 注入同一条所有权契约。部署切换因此保持纯配置：换部署只改 `Preset` 条目里的 `Implementation` 字符串，typed 对象始终留在宿主手里。根包内建的 memory/console/local/smtp 不设工厂也不导出常量：它们的全部设置都可由扁平字符串表达（配置通道已覆盖），没有需要工厂携带的 typed 输入。与工厂互补的一站式构造器（`FromAddr`/`FromConfig`）另有其位：宿主手上只有一个地址或一份 typed `Config`、不打算共享对象时，一次调用就拿到注入对，不必先手建再包工厂；带凭据/TLS/共享 client 的 typed 场景仍归工厂或直接 `New*`。端到端证明在 `go/pkgcore/unittest/factory_registration_test.go`（一个共享 client 走两个 seam 的完整回路：注册别名 → preset 引用 → `Bootstrap` 解析 → 经解析值触达宿主 client → `Shutdown` 不动它）。
...
"短信发送"一行是真实代码，不是设计目标：`go/pkgcore/sms.go` 的 `SMSSender` 是共享 seam（go/authn 的短信登录码与 go/notification 的 sms 渠道共用；无内核座位，各模块经自己的 `WithSMSSender` 注入），根包有两套实现——单进程用 `pkgcore.NewConsoleSMSSender`（写到注入的 `io.Writer`，`examples/reference-app` 默认接 `os.Stdout`），分布式用 `pkgcore.NewHTTPSMSSender`（通用 JSON 网关 POST，默认走 `pkgcore/safehttp` 的 SSRF 防护客户端）；authn 在分布式部署模式下不传 `SMSSender` 会在其装配期直接失败（`ErrMissingDistributedSMSSender`），不会静默退化成打印到 stdout。真正的运营商适配器在 `pkgcore/sms/`（`aliyun`、`tencent`、`twilio` 三个子包，每个一个 `NewSender` 构造器返回 `pkgcore.SMSSender`，由宿主照常装配）：各自按官方 API 做真实签名与请求，全部标准库手写、零新增依赖。仓库自身从未在真实网关前跑过这三个适配器，对真实账号的验收以 `ALIYUN_SMS_*`/`TENCENT_SMS_*`/`TWILIO_SMS_*` 环境变量门控的集成 leg 形式存在（缺凭据时带说明自跳过，alipay 沙箱 leg 的先例），见 `go/pkgcore/AGENTS.md` 的 "SMS carrier adapters" 一节。
...
- **`KVStore` seam 的 NATS JetStream KV 实现 `kv.nats`（`go/pkgcore/kv/nats`）**：`NewKVStore(ctx, nc, bucket)` 用一个已连接的 `*nats.Conn` 与一个 bucket 名构造，构造期用 `CreateOrUpdateKeyValue` 幂等地把 bucket 建好（这一点与 `kv/redis` 不同——`redis.NewKVStore` 不拨号，JetStream 的 bucket 是服务端必须先认可的 schema，不是客户端可以凭空假设存在的裸命名空间，所以这个构造函数要带 `ctx` 且可能失败）。JetStream KV 只有按版本号比较的 `Update(key, value, revision)`，没有原生按值比较、也没有原生原子自增，`CompareAndSwap`/`IncrByFloat` 因此都建在这一个版本号校验之上：读到的版本号只决定「接下来该发 `Create` 还是发 `Update`」，真正的原子性来自写入那一步服务端做的版本号校验——`Create` 撞见 `ErrKeyExists`、`Update` 撞见 `ErrKeyRevisionMismatch` 都被当作「swap 没发生」（`false, nil`），从不当错误；`IncrByFloat` 把同样的判断包进一个有限重试的 CAS 循环（上限 200 次、指数退避封顶 20ms）。过期时间是这层实现自己在被存值前加的信封（8 字节大端 Unix 纳秒头 + 原始值），从不使用 bucket 级 TTL——bucket TTL 是全 bucket 一个值，表达不了"自增在续命现有 key 的过期时间上创建新 key 时却不带过期时间"这条 `KVStore` 接口硬性要求的语义。JetStream 的 key 字符集不含冒号，而 `KVStore` 的 key 是不做限制的不透明字符串（`kvstoretest`/`kv/redis` 自己的测试就大量使用 `billing:invoice:1042` 这样的 key），所以这层实现把每个 key 十六进制编码后再落到 JetStream 上——无损、单射，对任何输入都合法。声明的能力位与 `kv.redis` 相同（`MultiReplicaSafe|SurvivesRestart`，默认 `FileStorage`），但 **`kv.nats` 没有被接进 `PresetDistributed`**——这个名字只在 `KVStoreRegistry` 上可解析，不改变任何既有 preset 的默认选择；一个想用 NATS 而非 Redis 或 PostgreSQL 的宿主把 `kv` 条目经配置通道指向它（`url`，可选 `bucket`），或注入一个已连接的 `*nats.Conn`。`go/pkgcore/kv/nats/integration_test/`（Docker-backed，JetStream-enabled 的真实 NATS 容器）跑通共享的 `kvstoretest.AssertConforms`，外加两个必需的对抗性并发证明——40 个 goroutine 各自 15 次 `IncrByFloat` 竞争同一个 key，最终和精确等于 600，没有一次触及重试上限；30 个 goroutine 竞争同一个从未写过的 key 的 `CompareAndSwap` set-if-absent，最终只有且恰好一个赢家——以及一个真实的服务器重启存活证明（同一容器 stop/start，客户端凭 `nats.go` 默认的自动重连恢复操作，且重启前写入的数据仍在，证实 `SurvivesRestart` 不只是声明）。**依赖成本（净增口径）**：`kv/nats` 对本仓库的净新增第三方依赖为零——`nats.go` 连同其间接依赖已是根 `go.mod` 的既有直接依赖（由 `eventbus/nats` 引入，两实现 pin 同一个 `github.com/nats-io/nats.go`），它复用的是同一份依赖闭包，而不是第二次单独付费。`.golangci.yml` 的 `nats-only-in-pkgcore` depguard 规则同时豁免 `eventbus/nats` 与 `kv/nats` 两个子包。
- **preset 的「参数通道」与「逐项覆盖」已按正文落地**：`Preset` 是 `map[string]SeamPreset`（`SeamPreset{Implementation, Config}`——实现名 + 该实现的参数），`resolveKernelSeam` 把条目里的 `Config` 原样交给该实现的 `Registration.New`，`seam_registry.go` 的 `Build(name, cfg)` 在 preset 路径上从此真正收到值：各内置实现文档化的配置键（redis 系的 `addr`/`password`/`db`、nats 系的 `url`/`token`/`user`/`password`/`bucket`、`mailer.smtp` 的 `host`/`port`/`username`/`password`/`tls_mode`/`insecure_skip_verify`、`objectstore.s3` 的 `endpoint`/`bucket`/`access_key`/`secret_key`/`region`/`use_ssl`、memcached 的 `addrs`、postgres 系的 `dsn`/`replica_id`）在 preset 路径上全部可达。逐项覆盖层的落地形态是 `Preset.With(seam, SeamPreset{...})`：返回副本，从不写穿调用方——`PresetStandalone`/`PresetDistributed` 是进程级共享映射，就地赋值会改写同一进程内所有后续 Kernel 的默认。`pkgcore` 自身仍从不读配置文件或环境：宿主自己的 koanf/环境层在上，把解析好的 `Preset` 喂进来（与 `Config.OTLPEndpoint` 的喂法相同，见 `preset.go` 的 doc comment）。内置 preset 的条目本身不带 `Config`——那是「命名实现，不携带部署」的诚实形状，pkgcore 不发布任何凭据——所以裸 `PresetDistributed` 下两个 Redis seam 仍落到裸默认地址（`localhost:6379`，无鉴权）、SMTP/S3 仍以 `ErrMissingSeamConfig` 解析失败；区别在于需要真实凭证的宿主不再被迫走代码注入：`PresetDistributed.With("mailer", SeamPreset{Implementation: "mailer.smtp", Config: Config{"host": ...}})` 即为「配置文件逐项覆盖」的落地形态（`examples/reference-app` 的 SMTP 与 S3 两个 seam 走的正是这条路径），需要共享既有资源或 typed 配置（TLS、池对象、共享 client）的宿主把对象注册进名字体系（各实现包导出的 `Registration` 工厂，见「组装的表达」一节），或照旧注入（`WithMailer`/`WithObjectStore`）——注入恒优先于 preset（按 seam）。参数通道的端到端证明在 `go/pkgcore/integration_test/kernel_lifecycle_test.go`：preset 条目携带真实容器的映射地址（自由端口，不再钉固定端口），解析出的实现拨到该地址、内核关闭时归还全部连接。装配校验、横幅警告与能力位已按正文落地。
