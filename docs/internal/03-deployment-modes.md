...
6. **每套实现独立成包；编译期引入哪些实现由应用组装者决定，框架不代劳**。约束 1 管源码层面（业务代码不 import 实现），这一条管打包层面（实现的依赖会不会落到接口消费者头上）——两者不能互相替代，因为 Go 按包而非按符号解析依赖，同包内联的实现无法被消费者裁剪。需要运行期在多套实现间切换的应用，把它们全部 import 进来即可，该能力一分不减。添加实现的 PR 必须附上实测的依赖增量。理由与测法见"实现注册表"节。
