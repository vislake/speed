...
**实施状态注记（本轮核实）：** 上表是设计意图的全景，不是现状——`go/jobs` 是目前唯一真正落地了自己那一行的模块：`queue_standalone.go` 通过 `otel.Meter` 注册了队列积压深度（`jobs.queue.depth`，一个按任务类型和状态分组的异步 Gauge 回调）、执行时长分位（`jobs.job.duration` Histogram）、尝试次数（`jobs.job.attempts` Counter）和死信计数（`jobs.job.dead_letter` Counter）四个 instrument，`worker.go` 在实际执行路径上写入后三者。其余六行——计量管道、通知、支付、AI 网关、站内信 SSE、认证——目前 `go/metering`、`go/notification`、`go/billing`、`go/ai-gateway`、`go/authn` 全文 grep `otel.Meter` 均为零命中，是模块已经落地但尚未补埋点，而不是模块本身不存在（`go/observability/AGENTS.md` 曾经的表述需要按这个区分读）。

**实施状态注记（2026-09，tooling 批次）：** 上一条注记的六行现状已大幅改变——认证、通知两行的埋点在此前轮次已各自落地（`authn.auth.count`/`authn.auth.duration`、notification 的投递计数与时长），本批次把剩余三行按模块真实拥有的点位补齐，每行映射与刻意保留的半行都记录在各自模块的 `metrics.go` doc comment 与 AGENTS.md 里：计量管道行（`go/metering/metrics.go`：`metering.events.ingested`/`metering.events.dropped` 于 `AnalyticsRecorder` 的 Record/drop 路径，`metering.outbox.delivery` 于 `Dispatcher` 的投递尝试，`metering.aggregation.duration` 于 `Aggregator` 两个 ingest 入口；"outbox 待投递积压"半行刻意用"采集−投递成功"推导而非深度 gauge——诚实深度需要每轮询对整张 outbox 表做 COUNT，批量有界的 claim 查询从不付这个扫描）；支付行（`go/billing/metrics.go` 加三个 gateway provider 的 `webhook_verify_metric`：`billing.invoice.transition`/`billing.invoice.open_dwell` 于发票状态迁移的实际落库点，`billing.webhook.verify` 于三家 provider 的 `VerifyWebhook`，channel 各自成系列；中间态订单计数由发票承载并记录推导理由）；AI 网关行（`go/ai-gateway/metrics.go`：`aigateway.provider.calls`/`.errors`/`.duration` 按路由解析出的 provider 与调用种类标注——chat/chat_stream 记在 `gateway.go` 的真实 provider 调用点，image 记在异步 job 的唯一咽喉 `callProvider`，流式终端错误在 `relayStream` 收到 Err 终结块时计数——加 `aigateway.rate_limited` 于 `checkRateLimit` 唯一拒绝点）。站内信 SSE 行仍未埋点（notification 模块的流端点，其投递计数已覆盖站内信写出侧；活跃连接数/扇出延迟/断连率仍未补），如实保留。各模块的 instrument 注册都带 nil-guard，测试走 ManualReader 断言语义，不依赖任何 collector。
